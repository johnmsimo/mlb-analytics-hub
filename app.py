...
def api_simulate(game_pk):
    try:
        ...
        # Try boxscore first; fall back to active roster so pre-game works
        away_lineup, home_lineup = [], []
        try:
            box = requests.get(f"{MLB_API}/game/{game_pk}/boxscore", timeout=10).json().get('teams', {})
            away_lineup = get_batters_from_boxscore(box.get('away', {}), 'away')
            home_lineup = get_batters_from_boxscore(box.get('home', {}), 'home')
            away_lineup = [b for b in away_lineup if b.get('name')]
            home_lineup = [b for b in home_lineup if b.get('name')]
        except Exception as ex:
            print(f"[simulate] boxscore error: {ex}")

        def _roster_batters(team_id, side):
            try:
                ...
                for entry in (r.json().get('roster') or []):
                    ...
                    out.append({
                        ...
                        'bats': fgb.get('fg_bats', 'R'),
                    })
                return out[:15]
            except Exception as ex:
                print(f"[simulate] roster fallback {side}: {ex}")
                return []

        if not away_lineup:
            away_lineup = _roster_batters(away_team_id, 'away')
        if not home_lineup:
            home_lineup = _roster_batters(home_team_id, 'home')

        if not away_lineup or not home_lineup:
            return jsonify({'success': False, 'error': 'Could not load lineups for either team'}), 400
...
def _mc_fetch_one(game_pk, away_tid, home_tid):
    """Fetch both lineups for one game — ONE boxscore call, roster fallback per empty side."""
    away_out, home_out = [], []
    try:
        r = requests.get(f"{MLB_API}/game/{game_pk}/boxscore", timeout=10)
        r.raise_for_status()
        box = r.json().get('teams', {})
        ab = get_batters_from_boxscore(box.get('away', {}), 'away')
        hb = get_batters_from_boxscore(box.get('home', {}), 'home')
        away_out = [b for b in ab if b.get('name')]
        home_out = [b for b in hb if b.get('name')]
    except Exception as ex:
        print(f"[mc] pk={game_pk} boxscore err: {ex}")

    def _roster(tid, side):
        try:
            r = requests.get(f"{MLB_API}/teams/{tid}/roster?rosterType=active", timeout=8)
            r.raise_for_status()
            out = []
            for e in (r.json().get('roster') or []):
                ...
                out.append({'name': name, 'slot': 0, 'side': side, 'pos': pos,
                    'fg_woba': fgb.get('fg_woba'), 'fg_wrc': fgb.get('fg_wrc'),
                    'fg_obp': fgb.get('fg_obp'), 'fg_slg': fgb.get('fg_slg'),
                    'fg_avg': fgb.get('fg_avg'), 'fg_kpct': fgb.get('fg_kpct'),
                    'sv_xba': svb.get('sv_xba'), 'sv_xwoba': svb.get('sv_xwoba'),
                    'sv_brl_pct': svb.get('sv_brl_pct'), 'sv_ev': svb.get('sv_ev'),
                })
            return out[:15]
        except Exception as ex:
            print(f"[mc] pk={game_pk} {side} roster err: {ex}")
            return []

    if len(away_out) < 5:
        away_out = _roster(away_tid, 'away')
    if len(home_out) < 5:
        home_out = _roster(home_tid, 'home')

    for b in away_out: b['side'] = 'away'
    for b in home_out: b['side'] = 'home'
    return away_out, home_out


def _mc_props_for_batters(batters, matchup):
    """Pure stat-model prop generation — no HTTP calls."""
    props = []
    for b in batters:
        name = b.get('name', '')
        if not name: continue
        ...
        for mk, line, prob, reason in [
            ...
            ('batter_total_bases', 1.5, pa_tb,
             f"xwOBA {woba:.3f} · SLG {fg_slg:.3f} · EV {ev:.1f} mph · wRC+ {wrc} → {pa_tb*100:.0f}% TB prob vs 1.5"),
            ...
            props.append({
                'player': name, 'market': mk, 'line': line,
                'bookmaker': 'Model', 'price': None,
                'edge': edge, 'prob': round(prob, 4),
                'source': 'simulation', 'matchup': matchup,
                'grade': _mc_grade(edge), 'recommendation': _mc_rec(edge),
                'reasoning': reason, 'slot': slot,
            })
    return props


def _mc_props_for_pitchers(pit_infos, matchup):
    props = []
    for pit_info in pit_infos:
        ...
        k_base = (k9 / 9 * 0.85) if k9 > 0 else 0.50
        k_prob = min(0.87, max(0.25, k_base + ((whiff-22)/100*0.4 if whiff>22 else 0) + ((4.10-xfip)/100*0.3)))
        k_reason = (f"K/9 {k9:.1f} · xFIP {xfip:.2f} · Whiff% {whiff:.1f}% · BB/9 {bb9:.1f}"
                    + (f" · {ip:.0f} IP" if ip > 0 else '') + f" → {k_prob*100:.0f}% K prob vs 5.5")
        if k_prob >= 0.30:
            edge = round(k_prob - 0.50, 4)
            props.append({
                'player': pname, 'market': 'pitcher_strikeouts', 'line': 5.5,
                'bookmaker': 'Model', 'price': None,
                'edge': edge, 'prob': round(k_prob, 4),
                'source': 'simulation', 'matchup': matchup,
                'grade': _mc_grade(edge), 'recommendation': _mc_rec(edge),
                'reasoning': k_reason, 'slot': 0,
            })
    return props
...
def api_projections_monte_carlo():
    """Monte Carlo projection board — parallel lineup fetches, stat-model props
    for every batter + both starters across all games today."""
    _maybe_refresh_fg()
    _maybe_refresh_savant()
    try:
        date_str = datetime.now(ET).strftime('%Y-%m-%d')
        ...
        # ── Parallel lineup fetch — all games at once ─────────────────────
        game_metas = []
        for g in raw:
        ...
        lineups_by_pk = {}
        def _fetch_one_game(meta):
            pk = meta['game_pk']
            a, h = _mc_fetch_one(pk, meta['away_tid'], meta['home_tid'])
            return pk, a, h

        with ThreadPoolExecutor(max_workers=8) as ex:
            futures = {ex.submit(_fetch_one_game, m): m for m in game_metas}
            for fut in as_completed(futures):
                try:
                    pk, a, h = fut.result()
                    lineups_by_pk[pk] = (a, h)
                except Exception as ex2:
                    print(f"[mc] lineup future error: {ex2}")

        # ── Build props ────────────────────────────────────────────────────
        games, ranked = [], []
        for meta in game_metas:
            game_pk  = meta['game_pk']
            matchup  = f"{meta['away']} @ {meta['home']}"
            top_props = []
            try:
                away_lineup, home_lineup = lineups_by_pk.get(game_pk, ([], []))

                if has_odds:
                    valid_names = set(b.get('name') for b in away_lineup + home_lineup if b.get('name'))
                    if meta['away_p'].get('fullName'): valid_names.add(meta['away_p']['fullName'])
                    if meta['home_p'].get('fullName'): valid_names.add(meta['home_p']['fullName'])
                    event, _ = _find_odds_event(meta['away_name'], meta['home_name'])
                    props_books = _load_event_odds(event.get('id') if event else None, featured_only=False) if event else []
                    props = _parse_prop_markets(props_books, valid_names)
                    scored = []
                    for p in props:
                    ...
                    print(f"[mc] {matchup}: {len(away_lineup)} away + {len(home_lineup)} home batters")
                    top_props  = _mc_props_for_batters(away_lineup + home_lineup, matchup)
                    top_props += _mc_props_for_pitchers([meta['away_p'], meta['home_p']], matchup)
