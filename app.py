...
def _load_fg_data():
    ...
    try:
        ...
        # Log actual column names so we can debug key mismatches on Render
    ...
    # ── Commit to cache ───────────────────────────────────────────────────
    # Only mark loaded when pitching cache actually has data.
    # If pitching failed, _fg_loaded stays False so _maybe_refresh_fg retries.
    with _fg_lock:
        if bat:
            _fg_bat = bat
        if pit:
...
def _maybe_refresh_fg():
    ...
    # Retry if never loaded, stale date, or pitcher cache still empty
