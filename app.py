...
def _mc_compute_background():
    ...
    try:
        ...
        # ── Build per-game lineup: schedule card → boxscore → roster ──────
        def _parse_sched(hitters, side):
            out = []
        ...
        def _roster_lu(tid, side):
            try:
                ...
                for e in (r.json().get('roster') or []):
                    ...
                    if name: out.append({'name': name, 'slot': 5, 'side': side})
                return out[:15]
            except: return []
        ...
        def _get_lineups(g):
            ...
            status   = g.get('status', {}).get('abstractGameState', '')  # Preview/Live/Final

            lineups      = g.get('lineups') or {}
            away_hitters = lineups.get('awayBatters') or []
            home_hitters = lineups.get('homeBatters') or []
            away_lu = _parse_sched(away_hitters, 'away')
            home_lu = _parse_sched(home_hitters, 'home')

            # For live/final games or when schedule card is empty, use boxscore
            need_box = (status in ('Live', 'Final') or
                        len(away_lu) < 5 or len(home_lu) < 5)
            if need_box:
                box_away, box_home = [], []
                try:
                    ...
                    r.raise_for_status()
                    box = r.json().get('teams', {})
                    raw_a = get_batters_from_boxscore(box.get('away', {}), 'away')
                    raw_h = get_batters_from_boxscore(box.get('home', {}), 'home')
                    box_away = [b for b in raw_a if b.get('name')]
                    box_home = [b for b in raw_h if b.get('name')]
                    for b in box_away: b['side'] = 'away'
                    for b in box_home: b['side'] = 'home'
                except Exception as ex:
                    print(f"[mc_bg] pk={game_pk} boxscore err: {ex}")

                if len(box_away) >= 5: away_lu = box_away
                if len(box_home) >= 5: home_lu = box_home

            # Roster last resort
            if len(away_lu) < 5: away_lu = _roster_lu(away_tid, 'away')
            if len(home_lu) < 5: home_lu = _roster_lu(home_tid, 'home')

            print(f"[mc_bg] pk={game_pk} ({status}): {len(away_lu)}a + {len(home_lu)}h")
            return game_pk, away_lu, home_lu

        # Fetch all lineups in parallel
        lineup_map = {}
        with ThreadPoolExecutor(max_workers=8) as pool:
            futs = {pool.submit(_get_lineups, g): g.get('gamePk') for g in raw}
            for fut in as_completed(futs):
                try:
                    pk, alu, hlu = fut.result()
                    lineup_map[pk] = (alu, hlu)
                except Exception as ex:
                    print(f"[mc_bg] lineup future err: {ex}")
        ...
        for g in raw:
            ...
            top_props = []
            try:
                away_lu, home_lu = lineup_map.get(game_pk, ([], []))
...
def _mc_maybe_refresh(force=False):
    ...
    if running: return
    # Refresh if: forced, no data, or older than 10 min (600s)
    if not force and has_data and age < 600: return
