...
def _load_fg_data():
    ...
    try:
        ...
        # Log actual column names once so we can debug key mismatches
        print(f"[FG] Pitching columns: {list(df.columns[:30])}")
        for _, r in df.iterrows():
            k = str(r.get("Name","")).strip().lower()
            if not k:
                continue
            def _fv(col, dec=2):
                try:    return round(float(r.get(col) or 0), dec)
                except: return 0.0
            pit[k] = {
                "fg_era":   _fv("ERA",  2),
                "fg_fip":   _fv("FIP",  2),
    ...
    # ── Commit to cache ───────────────────────────────────────────────────
    # Only mark as loaded when pitching cache actually has data.
    # If pitching failed, leave _fg_loaded=False so _maybe_refresh_fg
    # will keep retrying on the next API call.
    with _fg_lock:
        if bat:
            _fg_bat = bat
        if pit:
            ...
            print(f"[FG] Cache committed — {len(bat)} batters, {len(pit)} pitchers")
        else:
            print("[FG] Pitching cache empty — will retry on next request")

def _maybe_refresh_fg():
    with _fg_lock:
        loaded    = _fg_loaded
        date      = _fg_load_date
        pit_count = len(_fg_pit)
    # Retry if: never loaded, stale date, or pitcher cache empty
    if not loaded or date != datetime.now().date() or pit_count == 0:
