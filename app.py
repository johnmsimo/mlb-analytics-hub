...
def api_gameside_deepdive(game_pk):
    """Composite endpoint that aggregates everything the gameside
    deepdive UI needs in one call: game meta, pitcher stats with
    arsenal, opposing lineup ranked by damage score, and wind/run
    environment scoring."""
    ...
    try:
        ...
        for g in raw:
            if g.get("gamePk") == game_pk:
                gmeta = parse_game(g); break
        if not gmeta:
            return jsonify({"success": False, "error": "Game not found"}), 404

        # Pitcher data (reuse existing helper)
        box = requests.get(f"{MLB_API}/game/{game_pk}/boxscore", timeout=10).json()
        away_bats = get_batters_from_boxscore(box.get("teams", {}).get("away", {}), "away")
        home_bats = get_batters_from_boxscore(box.get("teams", {}).get("home", {}), "home")

        def build_p(name, pid):
            m = pitcher_stats_mlb(pid) if pid else {}
            f = fg_pitcher(name) or {}
            s = sv_pitcher(name) or {}
            out = dict(m); out.update(f)
            for k, v in s.items():
                if k not in ("sv_arsenal_pct", "sv_arsenal_velo"):
                    out[k] = v
            out["sv_arsenal_pct"] = s.get("sv_arsenal_pct", {})
            out["sv_arsenal_velo"] = s.get("sv_arsenal_velo", {})
            return out

        # probable pitchers from schedule hydrate
        ap = hp = {}
        for g in raw:
            if g.get("gamePk") == game_pk:
                ...
                break
        an, hn = ap.get("fullName", "TBD"), hp.get("fullName", "TBD")
        away_p = {"id": ap.get("id"), "name": an, "stats": build_p(an, ap.get("id"))}
        home_p = {"id": hp.get("id"), "name": hn, "stats": build_p(hn, hp.get("id"))}

        # Rank opposing batters by damage score
        def rank_vs(batters, pitcher_stats):
            out = []
            for b in batters:
                fgb = fg_batter(b["name"]) or {}
                svb = sv_batter(b["name"]) or {}
                merged = dict(b); merged.update(fgb); merged.update(svb)
                merged["damage_score"] = _damage_score(merged, pitcher_stats)
                out.append(merged)
