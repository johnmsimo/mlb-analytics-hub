...
def api_simulate(game_pk):
    try:
        ...
        _box_r = requests.get(f"{MLB_API}/game/{game_pk}/boxscore", timeout=10)
        if not _box_r.ok:
            return jsonify({'success': False, 'error': f'MLB API error {_box_r.status_code} fetching boxscore'}), 502
        try:
            box = _box_r.json().get('teams', {})
        except Exception:
            return jsonify({'success': False, 'error': 'MLB API returned malformed boxscore response'}), 502
...
def _build_tracker_rows_for_game(game_pk, capture_date, adjustments=None):
    ...
    try:
        _tb_r = requests.get(f"{MLB_API}/game/{game_pk}/boxscore", timeout=10)
        _tb_r.raise_for_status()
        box = _tb_r.json().get('teams', {})
    except Exception as _tb_ex:
        print(f'[tracker_boxscore] gamePk={game_pk} err={_tb_ex}')
        return []
    away_lineup = get_batters_from_boxscore(box.get('away', {}), 'away')
    home_lineup = get_batters_from_boxscore(box.get('home', {}), 'home')
    if not away_lineup or not home_lineup:
...
def api_tracker_capture(date_str):
    try:
        ...
        if sched is None:
            return jsonify({'success': False, 'error': 'Could not fetch schedule — MLB API may be unavailable'}), 502
        entries = []
        for g in sched:
            try:
                rows = _build_tracker_rows_for_game(g.get('gamePk'), date_str, adjustments)
                if rows:
                    entries.extend(rows)
            except Exception:
        ...
        print('[api_tracker_capture]', traceback.format_exc())
        return jsonify({'success': False, 'error': f'Capture failed: {str(ex)}'}), 500
...
def api_build_parlay():
    ...
    try:
        ...
        for sel in selections:
            ...
            # MC-derived probability — uses sim data when passed from /api/simulate
            mc_prob = sel.get('mc_prob')
            if mc_prob and 0.05 < float(mc_prob) < 0.98:
                base_prob = round(float(mc_prob), 4)
            elif market in ('batter_hits', 'batter_home_runs', 'batter_rbis', 'batter_total_bases'):
                brl_pct = float(sel.get('brl_pct', 5.5))
                ev      = float(sel.get('ev', 87.5))
                woba    = float(sel.get('woba', 0.320))
                quality_boost = (brl_pct - 5.5)*0.003 + (ev - 87.5)*0.002 + (woba - 0.320)*0.4
                base_prob = max(0.35, min(0.72, 0.52 + quality_boost + (0.04 if projection < 1.5 else -0.03)))
            elif market == 'pitcher_strikeouts':
                k9   = float(sel.get('k9', 8.2))
                xera = float(sel.get('xera', 4.2))
                k_boost = (k9 - 8.2)*0.018 - (xera - 4.2)*0.012
                base_prob = max(0.35, min(0.72, 0.52 + k_boost + (0.05 if projection <= 5.5 else 0.0)))
            else:
                base_prob = 0.52
...
def _build_ump_stats(ump_id, game_pks):
    ...
    return {
        ...
        "neural_edge_score": round(
            (k_vs_avg * 2.8) + (bb_vs_avg * -1.5) + (r_vs_avg * -2.0) + (min(n, 20) / 20 * 5)
        , 1),
        "neural_edge_label": (
            "STRONG K EDGE"   if k_vs_avg >= 1.5 else
            "MODERATE K EDGE" if k_vs_avg >= 0.7 else
            "HITTER LEAN"     if r_vs_avg >= 0.5 else
            "NEUTRAL"
        ),
        "prop_edges": {
            "k_overs":   round(0.50 + k_vs_avg  * 0.035, 3),
            "k_unders":  round(0.50 - k_vs_avg  * 0.035, 3),
            "run_overs": round(0.50 + r_vs_avg   * 0.030, 3),
        },
    }
...
def _load_ump_data(ump_id, ump_name):
    ...
    season    = today.year
    # FIXED: was fetching entire season — now limited to last 45 days for speed
    start_dt  = max(today - timedelta(days=45), datetime(season, 3, 1).date())
    end_dt    = today - timedelta(days=1)
    start     = start_dt.strftime("%Y-%m-%d")
    end       = end_dt.strftime("%Y-%m-%d")
    if start > end:
        return None  # season just started, no data yet
...
@app.route("/api/umpire/preload/<int:game_pk>", methods=["POST"])
def api_umpire_preload(game_pk):
    """Eagerly kick off umpire data load without blocking. Call this when deep-dive opens."""
    try:
        for delta in (0, -1, 1):
            date_str = (datetime.now(ET) + timedelta(days=delta)).strftime("%Y-%m-%d")
        ...
        if gdata:
            ...
            if ump and ump.get("id"):
                _get_cached_ump(ump["id"], ump.get("fullName", "Unknown"))
                return jsonify({"success": True, "message": "Preload started"})
        return jsonify({"success": True, "message": "No umpire to preload"})
    except Exception as ex:
        return jsonify({"success": False, "error": str(ex)}), 500
...
# ── CSV Export ───────────────────────────────────────────────────────────────
@app.route('/api/tracker/export-csv/<date_str>')
def api_tracker_export_csv(date_str):
    """Export tracker entries for a date as a downloadable CSV."""
    try:
        store   = _tracker_store()
        day     = store.get(date_str, {})
        entries = day.get('entries', [])
        if not entries:
            return jsonify({'success': False, 'error': 'No entries for date'}), 404
        si = io.StringIO()
        fields = ['player','team','market','line','projection','edge','result',
                  'graded','correct','roi','mc_prob','hit_rate','hr_rate','k_rate']
        writer = csvmod.DictWriter(si, fieldnames=fields, extrasaction='ignore')
        writer.writeheader()
        for e in entries:
        ...
        return Response(
            si.getvalue(),
            mimetype='text/csv',
            headers={'Content-Disposition': f'attachment; filename=tracker_{date_str}.csv'}
        )
    except Exception as ex:
        print(f"[export_csv] {traceback.format_exc()}")
        return jsonify({'success': False, 'error': str(ex)}), 500
...
def api_audit_summary():
    """Multi-day model performance summary for audit dashboard."""
    try:
        store = _tracker_store()
        rows  = []
        for date_str, day in sorted(store.items(), reverse=True)[:30]:
            entries = day.get('entries', [])
            graded  = [e for e in entries if e.get('graded')]
            if not graded:
                continue
            correct  = sum(1 for e in graded if e.get('correct'))
            total    = len(graded)
            hit_pct  = round(correct / total, 3) if total else 0
            avg_edge = round(sum(float(e.get('edge', 0) or 0) for e in graded) / total, 2) if total else 0
            roi_sum  = round(sum(float(e.get('roi', 0) or 0) for e in graded), 2)
            by_market = {}
            for e in graded:
                mk = e.get('market', 'unknown')
                by_market.setdefault(mk, {'total': 0, 'correct': 0})
                by_market[mk]['total'] += 1
                if e.get('correct'):
                    by_market[mk]['correct'] += 1
            rows.append({
                'date': date_str, 'total': total, 'correct': correct,
                'hit_pct': hit_pct, 'avg_edge': avg_edge, 'roi_sum': roi_sum,
                'by_market': by_market,
            })
        overall_total   = sum(r['total']   for r in rows)
        overall_correct = sum(r['correct'] for r in rows)
        overall_roi     = round(sum(r['roi_sum'] for r in rows), 2)
        return jsonify({
            'success': True,
            'summary': {
                'total_bets':    overall_total,
                'correct':       overall_correct,
                'hit_pct':       round(overall_correct / overall_total, 3) if overall_total else 0,
                'roi':           overall_roi,
        ...
        print(f"[api_audit_summary] {traceback.format_exc()}")
        return jsonify({'success': False, 'error': str(ex)}), 500


# ── Audit page ────────────────────────────────────────────────────────────────
_AUDIT_HTML_CACHE = [None]

def _get_audit_html():
    if _AUDIT_HTML_CACHE[0] is None:
        p = os.path.join(_HERE, 'audit.html')
        _AUDIT_HTML_CACHE[0] = open(p).read() if os.path.exists(p) else '<h1>audit.html missing</h1>'
    return _AUDIT_HTML_CACHE[0]

@app.route('/audit')
def audit_page():
    return _get_audit_html()
