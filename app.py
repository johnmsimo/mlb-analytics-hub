...
def api_batting_order_matchups(game_pk):
    """Return both lineups enriched with FG/Savant stats + platoon splits vs opposing pitcher."""
    _maybe_refresh_fg()
    _maybe_refresh_savant()
    try:
        # ── Get schedule to identify pitchers + teams ─────────────────────
        ...
        # Pitcher hand from MLB API
        def _pitcher_hand(pid):
            if not pid: return "R"
            try:
                r = requests.get(f"{MLB_API}/people/{pid}", timeout=6)
                r.raise_for_status()
                return r.json().get("people", [{}])[0].get("pitchHand", {}).get("code", "R")
            except:
                return "R"

        ap_hand = _pitcher_hand(ap.get("id"))
        hp_hand = _pitcher_hand(hp.get("id"))
        ...
        try:
            ...
            away_raw = get_batters_from_boxscore(box.get("away", {}), "away")
            home_raw = get_batters_from_boxscore(box.get("home", {}), "home")
        except:
            away_raw = []
            home_raw = []
        ...
        def _split_avg(player_id, vs_hand):
            """Fetch platoon split avg for a batter vs a pitcher hand."""
            if not player_id or not vs_hand:
                return None
            try:
                pr = requests.get(
                    f"{MLB_API}/people/{player_id}/stats",
                    params={"stats": "statSplits", "group": "hitting",
                            "season": year, "sitCodes": "vl,vr"},
                    timeout=6
                )
                pr.raise_for_status()
                code = "vl" if vs_hand.upper() == "L" else "vr"
                for sp in pr.json().get("stats", [{}])[0].get("splits", []):
                    if sp.get("split", {}).get("code", "") == code:
                        return sp.get("stat", {}).get("avg", None)
            except:
                pass
            return None

        def _enrich_batters(batter_list, opp_pitcher_hand):
            out = []
            for b in batter_list[:9]:
                name  = b.get("name", "")
                pid   = b.get("id")
                fgb   = fg_batter(name)
                svb   = sv_batter(name)
                ...
                # ISO = SLG - AVG
                slg_v = b.get("slg") or fgb.get("fg_slg") or 0
                avg_v = b.get("avg") or fgb.get("fg_avg") or 0
                try:
                    iso = round(float(slg_v) - float(avg_v), 3)
                except:
                    iso = 0.0

                split = _split_avg(pid, opp_pitcher_hand)
                ...
                out.append({
                    ...
                    "obp":       b.get("obp") or fgb.get("fg_obp") or "---",
                    "slg":       b.get("slg") or fgb.get("fg_slg") or "---",
                    "ops":       b.get("ops") or fgb.get("fg_ops") or "---",
                    "iso":       iso if iso > 0 else "---",
                    "xwoba":     svb.get("sv_xwoba") or "---",
                    "ev":        svb.get("sv_ev")    or "---",
                    "brl_pct":   svb.get("sv_brl_pct") or "---",
                    "split_avg": split or 0,
                })
            return out

        away_batters = _enrich_batters(away_raw, hp_hand)  # away faces home pitcher
        home_batters = _enrich_batters(home_raw, ap_hand)  # home faces away pitcher

        return jsonify({
            "success":         True,
            "gamePk":          game_pk,
            "awayAbbr":        away_t.get("team", {}).get("abbreviation", "AWAY"),
            "homeAbbr":        home_t.get("team", {}).get("abbreviation", "HOME"),
