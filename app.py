...
def _mc_lineups_for_game(game_pk, away_tid, home_tid):
    """Fetch both lineups with ONE boxscore call, roster fallback per missing side.
    Never makes more than 3 HTTP calls per game regardless of scenario."""
    away_out, home_out = [], []

    # --- ONE boxscore call covers both teams ---
    try:
        r = requests.get(f"{MLB_API}/game/{game_pk}/boxscore", timeout=10)
        r.raise_for_status()
        box = r.json().get('teams', {})
        ...
        away_named = [b for b in ab if b.get('name')]
        home_named = [b for b in hb if b.get('name')]
        if len(away_named) >= 5:
            away_out = away_named
        if len(home_named) >= 5:
            home_out = home_named
        print(f"[mc] pk={game_pk} boxscore: away={len(away_out)} home={len(home_out)}")
    except Exception as ex:
        print(f"[mc] pk={game_pk} boxscore error: {ex}")

    # --- Roster fallback only for sides still missing ---
    def _roster(team_id, side):
        try:
            ...
            for entry in (r.json().get('roster') or []):
                ...
                svb = sv_batter(name)
                out.append({
                    'name': name, 'slot': 0, 'pos': pos, 'side': side,
            ...
            print(f"[mc] pk={game_pk} {side} roster fallback: {len(out)} players")
            return out[:20]
        except Exception as ex:
            print(f"[mc] pk={game_pk} {side} roster error: {ex}")
            return []
...
def api_projections_monte_carlo():
    """Monte Carlo projection board — stat-model projections for all confirmed
    batters and both starters across every game today. Graded with reasoning."""
    _maybe_refresh_fg()
    _maybe_refresh_savant()
    try:
        date_str = datetime.now(ET).strftime('%Y-%m-%d')
        ...
        for g in raw:
            ...
            try:
                ...
                # Single combined call — max 3 HTTP calls per game
                away_lineup, home_lineup = _mc_lineups_for_game(game_pk, away_tid, home_tid)
                ...
                if has_odds:
                    ...
                    # Stat-model simulation path
                    all_batters = away_lineup + home_lineup
                    print(f"[mc] {matchup}: {len(all_batters)} total batters")
