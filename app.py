    ...
    try:
        ...
        try:
            ...
            if api_key:
                ...
                def fmt_pitcher(info, fg, sv, mlb_stats):
                    hand = mlb_stats.get('pitchHand', '?')
                    return (
                        f"  Name: {info['name']} ({hand}HP)\n"
                        f"  ERA: {info['era']} | xERA: {info['xera']} | FIP: {fg.get('fg_fip','N/A')} | xFIP: {fg.get('fg_xfip','N/A')}\n"
                        f"  WHIP: {info['whip']} | K/9: {info['k9']} | BB/9: {fg.get('fg_bb9','N/A')}\n"
                        f"  K%: {fg.get('fg_kpct','N/A')} | BB%: {fg.get('fg_bbpct','N/A')} | LOB%: {fg.get('fg_lob','N/A')}\n"
                        f"  BABIP: {fg.get('fg_babip','N/A')} | WAR: {fg.get('fg_war','N/A')} | IP: {fg.get('fg_ip','N/A')}"
                    )

                def fmt_batter(b):
                    return (
                        f"  {b.get('slot','?')}. {b.get('name','?')} ({b.get('pos','?')}) | "
                        f"AVG:{b.get('avg','---')} OBP:{b.get('obp','---')} SLG:{b.get('slg','---')} | "
                        f"wOBA:{b.get('woba','---')} xwOBA:{b.get('xwoba','---')} | "
                        f"EV:{b.get('ev','---')} | "
                        f"vsL:{b.get('vs_l_avg','---')} vsR:{b.get('vs_r_avg','---')}"
                    )

                def fmt_lineup(bats):
                    return "\n".join(fmt_batter(b) for b in bats if b.get('name'))

                # Fetch recent pitcher game logs for extra context
                def pitcher_recent_form(pid, name):
                    if not pid: return f"  {name}: No recent data"
                    try:
                        yr = datetime.now().year
                        r = requests.get(f"{MLB_API}/people/{pid}/stats",
                            params={"stats":"gameLog","group":"pitching","season":yr}, timeout=6)
                        splits = r.json().get("stats",[{}])[0].get("splits",[])[-4:]
                        lines = []
                        for s in reversed(splits):
                            st = s.get("stat",{})
                            lines.append(f"    {s.get('date','')[:10]} vs {(s.get('opponent')or{}).get('abbreviation','?')}: "
                                         f"{st.get('inningsPitched','?')} IP, {st.get('strikeOuts','?')} K, "
                                         f"{st.get('earnedRuns','?')} ER, {st.get('baseOnBalls','?')} BB")
                        return f"  {name} last {len(lines)} starts:\n" + "\n".join(lines) if lines else f"  {name}: No recent starts"
                    except Exception:
                        return f"  {name}: Recent form unavailable"

                ap_recent = pitcher_recent_form(ap_id, ap_name)
                hp_recent = pitcher_recent_form(hp_id, hp_name)

                # Build the prompt
                away_lu = fmt_lineup(context['away_lineup'])
                home_lu = fmt_lineup(context['home_lineup'])

                prompt = f"""You are an elite MLB betting analyst with deep knowledge of sabermetrics, props, and game theory.

GAME: {context['away_team']} ({context['away_abbr']}) @ {context['home_team']} ({context['home_abbr']})
...
═══ AWAY PITCHER: {context['away_pitcher']['name']} ═══
{fmt_pitcher(context['away_pitcher'], ap_fg, ap_sv, ap_stats)}

RECENT FORM:
{ap_recent}

═══ HOME PITCHER: {context['home_pitcher']['name']} ═══
{fmt_pitcher(context['home_pitcher'], hp_fg, hp_sv, hp_stats)}

RECENT FORM:
{hp_recent}

═══ AWAY LINEUP ({context['away_abbr']}) vs {context['home_pitcher']['name']} ═══
{away_lu if away_lu else '  Lineup not yet confirmed'}

═══ HOME LINEUP ({context['home_abbr']}) vs {context['away_pitcher']['name']} ═══
{home_lu if home_lu else '  Lineup not yet confirmed'}

Analyze ALL the above data holistically — recent pitcher form, platoon splits (vsL/vsR AVG shown), exit velocity, xwOBA vs wOBA divergence, park factor, and weather. 

Respond ONLY with this exact JSON (no markdown, no backticks, raw JSON):
{{
  "away_runs": <integer 1-12>,
  "home_runs": <integer 1-12>,
  "away_hits": <integer 4-18>,
  "home_hits": <integer 4-18>,
  "total_runs": <integer>,
  "away_reasoning": "<2-3 sentences: why this run total, citing specific stats>",
  "home_reasoning": "<2-3 sentences: why this run total, citing specific stats>",
  "key_factors": [
    "<specific factor 1 with actual numbers>",
    "<specific factor 2 with actual numbers>",
    "<specific factor 3 with actual numbers>"
  ],
  "confidence": "<HIGH|MEDIUM|LOW>",
  "ou_lean": "<OVER|UNDER|NEUTRAL>",
  "ou_reasoning": "<1 sentence explaining the total lean>",
  "notable_props": [
    {{
      "player": "<full name>",
      "team": "<abbr>",
      "prop": "<hits|hr|tb|k|rbi>",
      "line": <float like 0.5 or 1.5>,
      "side": "<Over|Under>",
      "projection": <float>,
      "confidence": "<HIGH|MEDIUM>",
      "reasoning": "<specific reason citing matchup stats, platoon, recent form>"
    }}
  ]
}}

Include 4-6 notable_props covering the best angles: top batters with platoon advantages, pitcher K props, any high-value unders. Be specific — cite the actual stats that drive your projection."""

                client = anthropic.Anthropic(api_key=api_key)
                response = client.messages.create(
                    model="claude-sonnet-4-20250514",
                    max_tokens=2500,
                    temperature=0.3,   # Lower temp = more consistent, data-driven
                    system="You are an expert MLB analyst and sports betting consultant. You produce precise, data-driven projections grounded in sabermetrics. You always respond with raw JSON only — no preamble, no markdown, no backticks.",
            ...
            claude_error = str(ex)
            print(f"[ai_boxscore_claude] {claude_error}")
        ...
        return jsonify({
            ...
            'lineups': {
                'away': context['away_lineup'],
                'home': context['home_lineup'],
            },
...
# ── Route: AI Interactive Chat for a game ────────────────────────────────────
@app.route('/api/ai-chat/<int:game_pk>', methods=['POST'])
def api_ai_chat(game_pk):
    """
    Interactive Claude chat for a specific game.
    Accepts: { question: str, history: [{role, content}], game_context: {...} }
    ...
    try:
        ...
        game_context = payload.get('game_context', {})     # from AI_DATA on the frontend

        # Build system prompt with game context
        matchup  = game_context.get('matchup', f'Game {game_pk}')
        venue    = (game_context.get('venue') or {}).get('name', '')
        pf       = (game_context.get('venue') or {}).get('park_factor', 1.0)
        weather  = game_context.get('weather') or {}
        pitching = game_context.get('pitching_matchup') or {}
        ap       = pitching.get('away_pitcher') or {}
        hp       = pitching.get('home_pitcher') or {}
        proj     = game_context.get('projections') or {}
        lineups  = game_context.get('lineups') or {}

        def lineup_summary(bats, abbr):
            if not bats: return f"{abbr}: lineup not confirmed"
            return ", ".join(
                f"{b.get('slot','?')}.{b.get('name','?')}({b.get('pos','?')})"
                for b in bats[:9] if b.get('name')
            )

        system_prompt = f"""You are an elite MLB analyst and betting consultant with deep expertise in sabermetrics, game theory, and sports betting markets. You are analyzing a specific game for a sharp bettor.

GAME CONTEXT:
Matchup: {matchup}
Venue: {venue} | Park Factor: {pf}x
Weather: {weather.get('temp','N/A')}°F | {weather.get('condition','')} | Wind: {weather.get('wind','N/A')}

PITCHING MATCHUP:
Away SP: {ap.get('name','TBD')} — ERA:{ap.get('era','N/A')} xERA:{ap.get('xera','N/A')} WHIP:{ap.get('whip','N/A')} K/9:{ap.get('k9','N/A')}
Home SP: {hp.get('name','TBD')} — ERA:{hp.get('era','N/A')} xERA:{hp.get('xera','N/A')} WHIP:{hp.get('whip','N/A')} K/9:{hp.get('k9','N/A')}

LINEUPS:
Away: {lineup_summary(lineups.get('away',[]), matchup.split(' vs ')[0] if ' vs ' in matchup else 'AWAY')}
Home: {lineup_summary(lineups.get('home',[]), matchup.split(' vs ')[1] if ' vs ' in matchup else 'HOME')}

MODEL PROJECTIONS:
Runs: {proj.get('away_runs','?')} - {proj.get('home_runs','?')} (Total: {proj.get('total_runs','?')})
O/U Lean: {proj.get('ou_lean', 'NEUTRAL')} — {proj.get('ou_reasoning','')}
Confidence: {proj.get('confidence','N/A')}

You have access to all game stats including platoon splits, xwOBA, EV, barrel rate, and recent pitcher form already analyzed. 

Respond conversationally but precisely — cite actual numbers when relevant. Be direct about bets you like and why. Keep answers concise (2-4 sentences per point). Use bullet points for multi-part answers."""
        ...
        print(f"[api_ai_chat] {traceback.format_exc()}")
        return jsonify({'success': False, 'error': str(ex)}), 500
