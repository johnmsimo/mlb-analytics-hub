...
# ── Monte Carlo lineup helper (3-tier: boxscore → schedule lineups → roster) ──
def _mc_fetch_lineup(game_pk, team_id, side):
    """Fetch batting lineup for Monte Carlo projections.
    Tier 1: boxscore (works live/post-game)
    Tier 2: schedule hydrate=lineups (works pre-game once lineup card is submitted)
    Tier 3: active roster filtered to position players (always available)
    """
    # Tier 1 — boxscore
    try:
        ...
        batters = get_batters_from_boxscore(box_teams.get(side, {}), side)
        named = [b for b in batters if b.get('name')]
        if len(named) >= 5:
            for b in named: b.setdefault('side', side)
            print(f"[mc_lineup] pk={game_pk} {side}: boxscore → {len(named)} batters")
            return named
    except Exception as ex:
        print(f"[mc_lineup] pk={game_pk} {side}: boxscore error: {ex}")

    # Tier 2 — schedule hydrate=lineups (pre-game confirmed lineups)
    try:
        r = requests.get(
            f"{MLB_API}/schedule",
        ...
        r.raise_for_status()
        dates = r.json().get('dates', [])
        gsched = dates[0].get('games', []) if dates else []
        gdata = next((gg for gg in gsched if gg.get('gamePk') == game_pk), None)
        if gdata:
            lineups_data = gdata.get('lineups') or {}
            # MLB API uses awayBatters / homeBatters
            key = 'awayBatters' if side == 'away' else 'homeBatters'
            hitters = lineups_data.get(key) or []
            if hitters:
                out = []
                for slot_i, p in enumerate(hitters, start=1):
                    ...
                    out.append({
                        ...
                        'fg_kpct': fgb.get('fg_kpct'), 'fg_obp': fgb.get('fg_obp'),
                        'fg_slg':  fgb.get('fg_slg'),
                        'sv_xba': svb.get('sv_xba'), 'sv_xwoba': svb.get('sv_xwoba'),
                        'sv_brl_pct': svb.get('sv_brl_pct'), 'sv_ev': svb.get('sv_ev'),
                    })
                if len(out) >= 5:
                    print(f"[mc_lineup] pk={game_pk} {side}: schedule/lineups → {len(out)} batters")
                    return out
    except Exception as ex:
        print(f"[mc_lineup] pk={game_pk} {side}: schedule/lineups error: {ex}")

    # Tier 3 — active roster (position players only)
    try:
        ...
        for entry in roster:
            ...
            svb = sv_batter(name)
            out.append({
                'name': name, 'slot': 0, 'side': side, 'pos': pos,
                'avg': fgb.get('fg_avg', '.---'), 'obp': fgb.get('fg_obp', '.---'),
                'slg': fgb.get('fg_slg', '.---'),
                'fg_woba': fgb.get('fg_woba'), 'fg_wrc': fgb.get('fg_wrc'),
                'fg_kpct': fgb.get('fg_kpct'), 'fg_obp': fgb.get('fg_obp'),
        ...
        print(f"[mc_lineup] pk={game_pk} {side}: roster fallback → {len(out)} players")
        return out[:20]
    except Exception as ex:
        print(f"[mc_lineup] pk={game_pk} {side}: roster fallback error: {ex}")
...
def api_projections_monte_carlo():
    """Monte Carlo projection board — stat-model projections for every batter
    and both starters across all games today. Grade/reasoning included."""
    _maybe_refresh_fg()
    _maybe_refresh_savant()
    try:
        date_str = datetime.now(ET).strftime('%Y-%m-%d')
        ...
        for g in raw:
            ...
            try:
                ...
                # Robust 3-tier lineup fetch
                away_lineup = _mc_fetch_lineup(game_pk, away_tid, 'away')
                home_lineup = _mc_fetch_lineup(game_pk, home_tid, 'home')
                ...
                if has_odds:
                    ...
                    for p in props:
                        ...
                        scored.append({
                            ...
                            'source': 'odds', 'grade': _mc_grade(edge),
                            'recommendation': _mc_rec(edge), 'reasoning': '',
                            'matchup': matchup,
                        })
                    top_props = sorted(scored, key=lambda x: x['edge'], reverse=True)[:12]

                else:
                    # --- Simulation / stat-model path ---
                    ...
                    for b in all_batters:
                        ...
                        # Always re-fetch from cache so all tiers get full stats
                        svb  = sv_batter(name)
                        fgb  = fg_batter(name)
                        ...
                        try: fg_slg = float(fgb.get('fg_slg')    or 0.380)
                        except: fg_slg = 0.380
                        ...
                        # Lineup position multiplier (more PAs near top of order)
                        ...
                        defs = [
                            ('batter_hits',        0.5, pa_hit,
                             f"xBA {xba:.3f} · wOBA {woba:.3f} · wRC+ {wrc} → {pa_hit*100:.0f}% hit prob vs 0.5 line; bats {slot} ({side})"),
                            ('batter_total_bases', 1.5, pa_tb,
                             f"xwOBA {woba:.3f} · SLG {fg_slg:.3f} · EV {ev:.1f} mph · wRC+ {wrc} → {pa_tb*100:.0f}% TB prob vs 1.5 line"),
                            ('batter_home_runs',   0.5, pa_hr,
                             f"Barrel% {brl:.1f} · EV {ev:.1f} mph · wOBA {woba:.3f} → {pa_hr*100:.0f}% HR prob vs 0.5 line"),
                            ('batter_rbis',        0.5, pa_rbi,
                             f"wOBA {woba:.3f} · OBP {obp:.3f} · bats {slot} ({side}) → {pa_rbi*100:.0f}% RBI prob vs 0.5 line"),
                        ]
                        for mk, line, prob, reason in defs:
                    ...
                    for pit_info in [away_p, home_p]:
                        ...
                        whiff_adj = (whiff - 22) / 100 * 0.4 if whiff > 22 else 0
                        xfip_adj  = (4.10 - xfip) / 100 * 0.3
                        k_prob = min(0.87, max(0.25, k_base + whiff_adj + xfip_adj))

                        k_reason = (
                            f"K/9 {k9:.1f} · xFIP {xfip:.2f} · Whiff% {whiff:.1f}% · BB/9 {bb9:.1f}"
                            + (f" · {ip:.0f} IP" if ip > 0 else '')
                            + f" → {k_prob*100:.0f}% K prob vs 5.5 line"
                        )
                        if k_prob >= 0.30:
                ...
                print(f"[mc_game] {game_pk} {matchup}: {traceback.format_exc()}")
        ...
        return jsonify({
            'success': True, 'date': date_str, 'hasOdds': has_odds,
