...
def _build_ai_lines(name, is_pitcher, season, fg, sv, logs):
    """Generate 3-5 plain-text AI scouting lines from cached + live stats."""
    lines = []
    try:
        ...
        if not is_pitcher:
            ...
            if avg > 0:
                ...
                lines.append(f"Batting {avg:.3f} this season ({tier}). Season wOBA {woba:.3f} ranks in the "
                              f"{'top third' if woba >= 0.340 else 'middle third' if woba >= 0.310 else 'bottom third'} of the league.")
            if wrc > 0:
                desc = "well above avg (elite)" if wrc >= 130 else "above avg" if wrc >= 110 else "roughly avg" if wrc >= 95 else "below avg"
                lines.append(f"wRC+ of {wrc} — {first} is {desc} in total offensive production (100 = league average).")
            if ev > 0:
                quality = "Elite contact quality — target power props." if ev >= 91 and brl >= 8 else "Solid contact." if ev >= 88 else "Soft contact profile — fade power markets."
            ...
            if xba > 0 and avg > 0:
                ...
                if abs(diff) >= 0.015:
                    ...
                    lines.append(f"xBA ({xba:.3f}) vs actual AVG ({avg:.3f}) gap of {diff:+.3f} suggests {first} is {direction}.")
            ...
            xfip = float(fg.get("fg_xfip") or 0)
            kpct = float(fg.get("fg_kpct") or 0)
            xera = float(sv.get("sv_xera") or 0)
            if era > 0:
                fip_note = f"FIP {fip:.2f} signals sustainable success." if fip <= era else f"FIP {fip:.2f} suggests ERA may rise."
                lines.append(f"ERA of {era:.2f} this season. {fip_note}")
            if xera > 0:
                if era < xera - 0.30:
                    lines.append(f"xERA {xera:.2f} — outperforming expected metrics, regression risk toward xERA.")
                elif era > xera + 0.30:
                    lines.append(f"xERA {xera:.2f} — underperforming expected metrics, positive regression candidate.")
                else:
                    lines.append(f"xERA {xera:.2f} closely aligns with ERA — sustainable performance.")
            if kpct > 0:
                k_desc = "elite strikeout pitcher — target K prop overs" if kpct >= 0.27 else "above-avg swing-and-miss" if kpct >= 0.22 else "below-avg K rate — fade K overs"
...
def api_player_profile(player_id):
    """Full player profile card: identity + season + FG/Savant cache + game log + splits + AI."""
    try:
        year = datetime.now().year

        # 1. Player identity + hydrated season stats
        pr = requests.get(
        ...
        # Season totals from hydrated stats
        season = {}
        ...
        # Strip underscores from keys so JS can access fg.fgavg, sv.svxba etc.
        fg_out = {k.replace("_", ""): v for k, v in fgr.items()}
        sv_out = {k.replace("_", ""): v for k, v in svr.items()}

        # 3. Game log (last 10 games) — single API call
        ...
        try:
            ...
            if lr.ok:
                ...
                for sp in (all_games[-10:] if len(all_games) >= 10 else all_games):
                    s   = sp.get("stat", {})
        ...
        # 4. Platoon splits — single API call
        platoon = {}
        try:
            plr = requests.get(
                f"{MLB_API}/people/{player_id}/stats",
        ...
        return jsonify({
            ...
            "pos": pos_code,  "team": team_abbr,
            "isPitcher": is_pitcher,
            "throws": throws, "bats": bats_side,
            "season": season,
            "fg": fg_out,
            "sv": sv_out,
            "gameLogs": game_logs,
...
def api_teams_overview():
    ...
    try:
        ...
        def fetch_roster(t):
            # ZERO per-player API calls — uses only pre-cached FG/Savant in-memory data
            tid = t.get('id')
            try:
            ...
            for r in roster[:40]:
                try:
                    ...
                    if pos == 'P':
                        ...
                        fg_kpct = fgp.get('fg_kpct') or svp.get('sv_k_pct')
                        fg_fip  = fgp.get('fg_fip') or fgp.get('fg_xfip')
                        stat_line = {
                            'label1': 'ERA',
                    ...
                    pass  # skip bad player entry, never crash the whole roster
            return {
                'id': tid,
