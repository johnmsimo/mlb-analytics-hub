...
def _mc_fetch_lineup(game_pk, team_id, side):
    """Three-tier lineup fetch for Monte Carlo:
      1. Boxscore (works live + post-game)
      2. Schedule hydrate=lineups (works pre-game once lineups are posted)
      3. Active roster fallback (always available, positional players only)
    Returns list of batter dicts with at least {name, slot, pos, side}.
    """
    # ── Tier 1: Boxscore ─────────────────────────────────────────────────────
    try:
        ...
        # Only trust it if we got real named batters
        named = [b for b in batters if b.get('name')]
        if len(named) >= 5:
            for b in named: b['side'] = side
            print(f"[mc_lineup] {game_pk}/{side}: boxscore {len(named)} batters")
            return named
    except Exception as ex:
        print(f"[mc_lineup] {game_pk}/{side}: boxscore failed: {ex}")

    # ── Tier 2: Schedule hydrate=lineups ─────────────────────────────────────
    try:
        r = requests.get(
            f"{MLB_API}/schedule",
        ...
        dates = r.json().get('dates', [])
        if dates:
            games_sched = dates[0].get('games', [])
            gdata = next((gg for gg in games_sched if gg.get('gamePk') == game_pk), None)
            if gdata:
                lineups = gdata.get('lineups') or {}
                hitters = lineups.get(f'{side}Batters') or lineups.get(f'{side[0].upper()+side[1:]}Batters') or []
                if hitters:
                    out = []
                    for slot_idx, p in enumerate(hitters, start=1):
                        ...
                        out.append({
                            ...
                            'obp':         fgb.get('fg_obp', '.---'),
                            'slg':         fgb.get('fg_slg', '.---'),
                            'fg_woba':     fgb.get('fg_woba'),
                            'fg_wrc':      fgb.get('fg_wrc'),
                            'fg_kpct':     fgb.get('fg_kpct'),
                            'sv_xba':      svb.get('sv_xba'),
                            'sv_xwoba':    svb.get('sv_xwoba'),
                            'sv_brl_pct':  svb.get('sv_brl_pct'),
                            'sv_ev':       svb.get('sv_ev'),
                        })
                    if len(out) >= 5:
                        print(f"[mc_lineup] {game_pk}/{side}: schedule/lineups {len(out)} batters")
                        return out
    except Exception as ex:
        print(f"[mc_lineup] {game_pk}/{side}: schedule/lineups failed: {ex}")

    # ── Tier 3: Active roster fallback ───────────────────────────────────────
    try:
        ...
        print(f"[mc_lineup] {game_pk}/{side}: roster fallback {len(out)} players")
        return out[:20]
    except Exception as ex:
        print(f"[mc_lineup] {game_pk}/{side}: roster fallback failed: {ex}")
            ...
            try:
                ...
                if has_odds:
                    ...
                    for b in all_batters:
                        ...
                        try: k_pct = float(fgb.get('fg_kpct') or 0.22)
                        except: k_pct = 0.22

                        # Lineup position PA bonus
                        lm = 1.06 if slot <= 2 else (1.03 if slot <= 5 else (0.97 if slot >= 8 else 1.0))

                        pa_prob_hit = min(0.92, xba * 3.2 * lm)
                        pa_prob_tb  = min(0.88, woba * 2.4 * lm)
                        pa_prob_hr  = min(0.50, brl / 100 * 1.8 * lm)
                        pa_prob_rbi = min(0.72, woba * 2.0 * lm)

                        hit_reason = (
                            f"xBA {xba:.3f} · wOBA {woba:.3f} · wRC+ {wrc} → "
                            f"{pa_prob_hit*100:.0f}% hit prob vs 0.5 line; bats {slot}{'' if not side else ' ('+side+')'}"
                        )
                        tb_reason = (
                            f"xwOBA {woba:.3f} · SLG {fg_slg:.3f} · EV {ev:.1f} mph · wRC+ {wrc} → "
                            f"{pa_prob_tb*100:.0f}% total-bases prob vs 1.5 line"
                        )
                        hr_reason = (
                            f"Barrel% {brl:.1f} · EV {ev:.1f} mph · wOBA {woba:.3f} → "
                            f"{pa_prob_hr*100:.0f}% HR prob vs 0.5 line; "
                            + ("elite barrel rate" if brl >= 15 else "above-avg power" if brl >= 8 else "limited power upside")
                        )
                        rbi_reason = (
                            f"wOBA {woba:.3f} · OBP {obp:.3f} · bats {slot}th → "
                            f"{pa_prob_rbi*100:.0f}% RBI prob vs 0.5 line"
                        )

                        hit_defs = [
                            ('batter_hits',        0.5, pa_prob_hit, hit_reason),
                            ('batter_total_bases', 1.5, pa_prob_tb,  tb_reason),
                            ('batter_home_runs',   0.5, pa_prob_hr,  hr_reason),
                            ('batter_rbis',        0.5, pa_prob_rbi, rbi_reason),
                        ]
                        for (mk, line, prob, reason) in hit_defs:
                            ...
                            top_props.append({
                                ...
                                'prob':           round(prob, 4),
                                'source':         'simulation',
                                'grade':          _mc_grade(edge),
                                'recommendation': _mc_rec(edge),
                                'reasoning':      reason,
                                'slot':           slot,
                            })
                    ...
                    for pit_info in [away_p, home_p]:
                        ...
                        try: era   = float(fgp.get('fg_era')   or 4.50)
                        except: era = 4.50
                        ...
                        k_quality = "elite" if k9 >= 11 else ("strong" if k9 >= 9 else "average")
                        k_reason = (
                            f"K/9 {k9:.1f} ({k_quality}) · xFIP {xfip:.2f} · Whiff% {whiff:.1f}% · "
                            f"BB/9 {bb9:.1f} → {k_prob*100:.0f}% K probability vs 5.5 line"
                            + (f" · {ip:.0f} IP this season" if ip > 0 else "")
                        )
                        if k_prob >= 0.30:
