    ...
    try:
        ...
        def _roster_batters(team_id):
            try:
                ...
                print(f"[simulate] roster {team_id}: {ex}")
                return []
        ...
        if not away_lineup or not home_lineup:
            return jsonify({'success': False, 'error': 'Could not load lineups'}), 400
...
_mc_cache_data  = None   # last successful result dict
_mc_cache_ts    = None   # datetime of last successful compute
_mc_computing   = False  # True while background thread is running
...
def _mc_batter_props(name, slot, side, matchup):
    """Stat-model props for one batter — pure cache lookups, no HTTP."""
...
def _mc_pitcher_props(pit_info, matchup):
...
def _mc_get_roster(team_id, side):
    """Position players from active roster — no stat lookup, just names/positions."""
    try:
        r = requests.get(f"{MLB_API}/teams/{team_id}/roster?rosterType=active", timeout=6)
        r.raise_for_status()
        out = []
        for e in (r.json().get('roster') or []):
            ...
            if pos in ('P', 'SP', 'RP', 'CP'): continue
            name = ((e.get('person') or {}).get('fullName') or '').strip()
            if name: out.append({'name': name, 'slot': 0, 'side': side, 'pos': pos})
        return out[:15]
    except Exception as ex:
        print(f"[mc] roster {team_id}/{side}: {ex}")
        return []

def _mc_compute_background():
    """Heavy computation — runs in background thread, updates _mc_cache_data."""
    global _mc_cache_data, _mc_cache_ts, _mc_computing
    try:
        date_str = datetime.now(ET).strftime('%Y-%m-%d')
        # ONE schedule call — hydrate=lineups gets lineup cards if posted
        url = (f"{MLB_API}/schedule?sportId=1&date={date_str}"
        ...
        for g in raw:
            ...
            try:
                ...
                away_lu = _parse_sched_hitters(away_hitters, 'away')
                home_lu = _parse_sched_hitters(home_hitters, 'home')

                # Roster fallback for missing sides (no extra HTTP if lineup was in schedule)
                if len(away_lu) < 5: away_lu = _mc_get_roster(away_tid, 'away')
                if len(home_lu) < 5: home_lu = _mc_get_roster(home_tid, 'home')
                ...
                if has_odds:
                    ...
                    for p in props_raw:
                        ...
                        top_props.append({'player': p.get('player'), 'market': p.get('market_key'),
                            'line': p.get('line'), 'bookmaker': p.get('bookmaker'),
                            'price': price, 'edge': round(edge,4), 'prob': round(fair,4),
                            'source': 'odds', 'matchup': matchup,
                            'grade': _mc_grade(edge), 'recommendation': _mc_rec(edge), 'reasoning': ''})
                    top_props = sorted(top_props, key=lambda x: x['edge'], reverse=True)[:12]
                else:
                    for b in all_batters:
                        top_props.extend(_mc_batter_props(b.get('name',''), b.get('slot',5), b.get('side',''), matchup))
                    for pit in [away_p, home_p]:
                        top_props.extend(_mc_pitcher_props(pit, matchup))
        ...
        with _mc_cache_lock:
            global _mc_cache_data, _mc_cache_ts
            _mc_cache_data = result
            _mc_cache_ts   = datetime.now()

        print(f"[mc_bg] Done — {len(ranked)} total props across {len(games)} games")
...
def _mc_maybe_start_compute(force=False):
    """Start background compute if cache is stale or missing."""
    global _mc_computing
    with _mc_cache_lock:
        already_running = _mc_computing
        cache_age = (datetime.now() - _mc_cache_ts).total_seconds() if _mc_cache_ts else 9999
        stale = cache_age > 1800  # refresh every 30 min
        if already_running:
            return
        if not force and not stale and _mc_cache_data:
            return
        _mc_computing = True
...
def api_projections_monte_carlo():
    """Returns instantly from cache. Background thread does all the work.
    Frontend should auto-retry if computing=True."""
    force = request.args.get('refresh') == '1'
    _mc_maybe_start_compute(force=force)
    ...
    # No cache yet — return computing state so frontend shows spinner
    date_str = datetime.now(ET).strftime('%Y-%m-%d')
    return jsonify({'success': True, 'computing': True, 'date': date_str,
                    'hasOdds': bool(ODDS_API_KEY), 'games': [], 'topProps': [],
                    'message': 'Computing projections… auto-refreshing in 20s'})
