...
def _load_fg_data_from_mlb_api():
    """Derive batting/pitching cache from /api/v1/stats endpoints when
    FanGraphs is unavailable.  Designed to NEVER raise and NEVER break the
    worker: every player is wrapped in try/except and all list accesses
    are bounds-checked.  Populates the SAME keys as pybaseball so the
    rest of the app (fg_batter/fg_pitcher) doesn't need to change."""
    global _fg_bat, _fg_pit
    year = datetime.now().year
    print("[FG] Starting MLB-API-derived load…")

    # 1. Get every active MLB player id+name in one shot
    try:
        r = requests.get(
        ...
        return
    print(f"[FG] {len(people)} active players found")

    pitchers = [p for p in people if (p.get("primaryPosition") or {}).get("abbreviation") == "P"]
    batters  = [p for p in people if (p.get("primaryPosition") or {}).get("abbreviation") != "P"]

    def _fetch_season_stats(pid, group):
        try:
            rr = requests.get(
    ...
    with ThreadPoolExecutor(max_workers=10) as pool:
        futs = {pool.submit(_fetch_season_stats, p["id"], "pitching"): p for p in pitchers if p.get("id")}
        for fut in as_completed(futs):
            ...
            try:
                s = fut.result() or {}
                if not s:
                    # No season stats (rookie/rehab/unsigned) — skip silently.
                    continue
                ...
                er   = _safe_num(s.get("earnedRuns"), 0)
                ...
                # xFIP: substitute league-avg HR/FB (~11%) against this pitcher's
                # flyball count. If we don't have FB data, fall back to FIP.
                league_hrfb = 0.108
                fly_balls   = ao  # close enough without batted-ball classification
                exp_hr = fly_balls * league_hrfb if fly_balls > 0 else hr
                xfip = (round((13*exp_hr + 3*(bb + hbp) - 2*so) / ip + 3.10, 2)
                        if ip > 0 else fip)

                # K% / BB% off batters faced (fall back to PA ≈ IP*4.3)
                ...
                # BABIP: (H - HR) / (AB - K - HR + SF); SF not in API, treat as 0
                bip_den = ab - so - hr
                ...
                # WAR (rough FIP-scale): (league_FIP - pitcher_FIP) * IP/9 * 0.11
                # league_FIP ~4.00, 0.11 approximates runs-per-win scaling.
                ...
                pit_out[name.lower()] = {
                    ...
                    # GB/FB ratio — useful for damage projections vs batters.
                    "fg_gb_pct": round(go / (go + ao), 3) if (go + ao) > 0 else 0.45,
                }
            except Exception as ex:
                # NEVER let a single bad row break the whole cache build.
                print(f"[FG-pit-derived] pid={pid}: {ex}")

    # --- Batters ---
    with ThreadPoolExecutor(max_workers=10) as pool:
        futs = {pool.submit(_fetch_season_stats, p["id"], "hitting"): p for p in batters if p.get("id")}
        for fut in as_completed(futs):
            p = futs[fut]
            pid  = p.get("id")
            name = (p.get("fullName") or "").strip()
            ...
            try:
                ...
                # wOBA via FanGraphs 2024 linear weights (~MLB-avg .320 constant)
                # num = 0.696*BB + 0.726*HBP + 0.888*1B + 1.258*2B + 1.599*3B + 2.054*HR
                # den = AB + BB + SF + HBP (we drop IBB since MLB API rarely returns it)
                ...
                # wRC+ proxy: league avg wOBA ~.320, wOBA scale ~1.15.
                wrc  = round(((woba - 0.320) / 1.15 + 0.325) / 0.325 * 100) if woba > 0 else 100
                # BABIP actual: (H - HR) / (AB - K - HR + SF)
                bip_den = ab - so - hr + sf
                ...
                bat_out[name.lower()] = {
                    ...
                    # Side for handedness splits used by matchup projection.
                    "fg_bats":  ((p.get("batSide") or {}).get("code") or "R"),
                }
            except Exception as ex:
                print(f"[FG-bat-derived] pid={pid}: {ex}")
