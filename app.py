...
# ── Prop-specific park factors (keyed by MLB team ID) ─────────────────────────
# HR factors have the most variance; hits the least. Domes default to near-neutral.
PARK_FACTORS_HR = {
    115:1.28, 140:1.18, 143:1.14, 118:1.10, 110:1.07, 147:1.06, 113:1.06,
    145:1.03, 112:1.04, 111:1.02, 142:1.00, 138:0.97, 120:0.97, 144:0.96,
    121:0.96, 114:0.96, 116:0.94, 134:0.94, 108:0.94, 119:0.93, 136:0.92,
    133:0.90, 139:0.90, 141:0.90, 146:0.90, 158:0.90, 109:0.90, 117:0.90,
    135:0.86, 137:0.84,
}
PARK_FACTORS_HITS = {
    115:1.14, 111:1.12, 140:1.08, 143:1.06, 113:1.04, 118:1.03, 145:1.02,
    110:1.01, 138:1.01, 117:1.00, 141:1.00, 158:1.00, 109:1.00, 146:1.00,
    112:1.00, 142:0.99, 147:0.98, 114:0.98, 116:0.98, 139:0.98, 121:0.97,
    120:0.97, 108:0.97, 144:0.96, 134:0.96, 133:0.95, 136:0.94, 119:0.94,
    135:0.92, 137:0.91,
}
PARK_FACTORS_TB = {
    115:1.19, 111:1.09, 140:1.12, 143:1.09, 118:1.06, 113:1.05, 110:1.03,
    147:1.02, 145:1.02, 112:1.01, 138:1.00, 142:0.99, 117:0.98, 141:0.98,
    158:0.98, 109:0.98, 146:0.98, 139:0.98, 114:0.97, 116:0.96, 120:0.97,
    121:0.97, 144:0.96, 108:0.96, 134:0.95, 133:0.93, 119:0.94, 136:0.93,
    135:0.90, 137:0.88,
}

def get_prop_park_factor(home_team_id, prop_type):
    """Returns prop-specific park factor. prop_type: 'hits'|'hr'|'tb'|default"""
    tid = int(home_team_id) if home_team_id else 0
    if prop_type == 'hr':    return PARK_FACTORS_HR.get(tid,   PARK_FACTORS.get(tid, 1.0))
    if prop_type == 'hits':  return PARK_FACTORS_HITS.get(tid, PARK_FACTORS.get(tid, 1.0))
    if prop_type == 'tb':    return PARK_FACTORS_TB.get(tid,   PARK_FACTORS.get(tid, 1.0))
...
# ── Batter projection engine (v3 — platoon-aware + prop-specific park factors) ─
def _project_batter(batter, opp_pitcher_name, opp_pitcher_fg, opp_pitcher_sv,
    ...
    Uses platoon-blended avg/obp/slg/ops + prop-specific park factors.
    """
    ...
    if home_team_id:
        ...
        pf_rbi  = park_factor   # blended for RBI/runs
    else:
        pf_hits = park_factor
        pf_hr   = park_factor
        pf_tb   = park_factor
    ...
    # HR: prop-specific PF elevated further (HRs most park-sensitive prop)
    hr_pf     = min(1.35, pf_hr * 1.05)
        ...
        def enrich_batters(batters, opp_pfg, opp_psv, opp_pst, opp_abbr, opp_pname):
            ...
            for b in batters[:9]:
                ...
                proj = _project_batter(
                    ...
                    home_team_id=home_id   # ← prop-specific park factors
                )
                result.append({
                    "name":         name,
                    "team":         b.get("team", ""),
...
# ── Route: Park Factors (prop-specific) ──────────────────────────────────────
...
def _bt_metrics(entries):
    ...
    stakes  = [float(r["stakeDollars"]) for r in entries
               if r.get("stakeDollars") is not None and float(r.get("stakeDollars", 0)) > 0]
    ...
    kelly_vals = [float(r["fullKellyPct"]) for r in entries
                  if r.get("fullKellyPct") is not None and float(r.get("fullKellyPct", 0)) > 0]
    ...
    return {
        "total": total, "graded": graded, "wins": wins, "losses": losses, "pushes": pushes,
        "hit_rate": hit_rate, "avg_edge": avg_edge, "avg_kelly": avg_kelly,
        "total_staked": total_staked, "total_profit": total_profit, "roi": roi,
        "avg_clv": avg_clv, "clv_pos_rate": clv_pos_rate,
    }


def _safe_f_bt(val, default=None):
    try:
...
def api_backtest():
    """
    Replays graded tracker entries over a date window and returns performance
    broken down by market, edge bucket, probability bucket, park tier, and month.
    Query params: ?start=YYYY-MM-DD  ?end=YYYY-MM-DD  ?market=<key>  ?min_edge=<float>
    """
    try:
        today    = datetime.now(ET).date()
        ...
        for i in range(n_days):
            ...
            rows = [r for r in (store.get(ds) or {}).get("entries", [])
        ...
        if not entries:
            return jsonify({"success": True, "total": 0,
                ...
                "by_park_tier": {}, "by_month": {}, "overall": {}, "calibration": []})
        ...
        for mk in set(r.get("marketKey") for r in entries):
            by_market[mk] = _bt_metrics([r for r in entries if r.get("marketKey") == mk])
        ...
        by_edge = {}
        for r in entries:
            by_edge.setdefault(edge_bucket(r), []).append(r)
        ...
        by_prob = {}
        for r in entries:
            by_prob.setdefault(prob_bucket(r), []).append(r)
        by_prob_bucket = {k: _bt_metrics(v) for k, v in by_prob.items()}

        # By park tier (look up from schedule, cached per game)
        _gm_pf = {}
        def get_game_pf(gpk, date_str):
            if gpk in _gm_pf: return _gm_pf[gpk]
            try:
                raw = fetch_schedule(date_str)
                g   = next((x for x in raw if x.get("gamePk") == gpk), None)
                if g:
                    tid = g["teams"]["home"]["team"]["id"]
                    _gm_pf[gpk] = PARK_FACTORS.get(tid, 1.0)
                    return _gm_pf[gpk]
        ...
        for r in entries:
            ...
            by_park.setdefault(park_tier(pf), []).append(r)
        by_park_tier = {k: _bt_metrics(v) for k, v in by_park.items()}

        # By month
        by_month = {}
        for r in entries:
            by_month.setdefault((r.get("date") or "")[:7], []).append(r)
        by_month_out = {k: _bt_metrics(v) for k, v in sorted(by_month.items())}

        # Calibration: model prob vs actual hit rate
        calibration = []
        for pk, subset in sorted(by_prob.items()):
        ...
        return jsonify({
            "success": True, "start": start_str, "end": end_str,
            "total": len(entries), "overall": overall,
            "by_market": by_market, "by_edge_bucket": by_edge_bucket,
            "by_prob_bucket": by_prob_bucket, "by_park_tier": by_park_tier,
            "by_month": by_month_out, "calibration": calibration,
        })
