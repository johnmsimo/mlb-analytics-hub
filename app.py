...
# Python floats like NaN/Infinity are not valid JSON. Flask's default encoder
# serialises them as bare `NaN`/`Infinity` which browsers reject with
# "The string did not match the expected pattern". This provider sanitises
# every response payload before serialisation.
...
try:
    ...
    pass  # Flask < 2.2 fallback — _sanitize_nan still applied per-route
# ─────────────────────────────────────────────────────────────────────────────
...
try:
    ...
    if (_sk_major, _sk_minor) >= (1, 7):
        # Model was trained on sklearn 1.6.1 — predict() hangs on 1.7+ due to
        # pandas/validation incompatibility and kills the gunicorn worker.
        # Disable until model is retrained. Pin scikit-learn==1.6.1 in requirements.txt
        # to re-enable the ML blend.
        hits_model    = None
        hits_features = []
        print(f"[ML] sklearn {_sklearn.__version__} incompatible with model (trained 1.6.1) — formula only")
    else:
        hits_model    = joblib.load("models/hits_model.pkl")
        hits_features = joblib.load("models/hits_model_features.pkl")
        print("[ML] hits_model loaded OK")
except Exception as _ml_load_err:
    hits_model    = None
    hits_features = []
    print(f"[ML] Model not loaded — formula only: {_ml_load_err}")
...
def _load_fg_data():
    """Load FanGraphs season stats from pre-scraped CSVs.

    Season priority (most recent first — newer data overwrites older):
      2026 (primary, update weekly) → 2025 → 2024 → 2023

    This gives full wRC+, wOBA, FIP, xFIP, WAR from FanGraphs.
    Falls back to MLB Stats API if no CSV files are present.

    CSV files expected in:
      FanGraph/data/fg_batting_2023.csv  ...  fg_batting_2026.csv
      FanGraph/data/fg_pitching_2023.csv ...  fg_pitching_2026.csv
    """
    global _fg_bat, _fg_pit, _fg_loaded, _fg_load_date
    import os, pandas as pd
    year  = datetime.now().year
    # Oldest → newest so newer seasons overwrite older
    ...
    def _int(v, default=0):
        try:
            return int(float(v)) if str(v) not in ('', 'None', 'nan', '--.--', '---', '.---') else default
        except (TypeError, ValueError):
            return default

    def _parse_bat(row, season):
        slg = _flt(row.get("SLG"))
        avg = _flt(row.get("AVG"))
        return {
            "fg_avg":    _flt(row.get("AVG")),
            "fg_obp":    _flt(row.get("OBP")),
            "fg_slg":    slg,
            "fg_ops":    _flt(row.get("OPS")),
            "fg_woba":   _flt(row.get("wOBA")),
            "fg_wrc":    _int(row.get("wRC+")),
            "fg_pa":     _int(row.get("PA")),
            "fg_r":      _int(row.get("R")),
            "fg_hr":     _int(row.get("HR")),
            "fg_rbi":    _int(row.get("RBI")),
            "fg_sb":     _int(row.get("SB")),
            "fg_war":    _flt(row.get("WAR")),
            "fg_babip":  _flt(row.get("BABIP")),
            "fg_bbpct":  _flt(row.get("BB%")),
            "fg_kpct":   _flt(row.get("K%")),
            "fg_iso":    _flt(row.get("ISO", max(0.0, slg - avg))),
            "fg_season": season,
        }

    def _parse_pit(row, season):
        return {
            "fg_era":    _flt(row.get("ERA")),
            "fg_fip":    _flt(row.get("FIP")),
            "fg_xfip":   _flt(row.get("xFIP")),
            "fg_whip":   _flt(row.get("WHIP")),
            "fg_k9":     _flt(row.get("K/9")),
            "fg_bb9":    _flt(row.get("BB/9")),
            "fg_hr9":    _flt(row.get("HR/9")),
            "fg_kpct":   _flt(row.get("K%")),
            "fg_bbpct":  _flt(row.get("BB%")),
            "fg_babip":  _flt(row.get("BABIP")),
            "fg_lob":    _flt(row.get("LOB%")),
            "fg_war":    _flt(row.get("WAR")),
            "fg_ip":     _flt(row.get("IP")),
            "fg_g":      _int(row.get("G")),
            "fg_gs":     _int(row.get("GS")),
            "fg_w":      _int(row.get("W")),
            "fg_l":      _int(row.get("L")),
            "fg_season": season,
        }
    ...
    # Load CSVs — oldest first so newest season overwrites (best data wins)
    for season in seasons:
        bat_file = os.path.join(_HERE, "FanGraph", "data", f"fg_batting_{season}.csv")
        pit_file = os.path.join(_HERE, "FanGraph", "data", f"fg_pitching_{season}.csv")
        ...
        if os.path.exists(bat_file):
            try:
                ...
                for _, row in df.iterrows():
                    ...
                    if name:
                        bat[name] = _parse_bat(row, season)
                        added += 1
                print(f"[FG CSV] Batting  {season}: {added} players")
                csv_loaded = True
            except Exception as ex:
                print(f"[FG CSV] Batting  {season} failed: {ex}")
        ...
        if os.path.exists(pit_file):
            try:
                ...
                for _, row in df.iterrows():
                    name = str(row.get("Name", "")).strip().lower()
                    if name:
                        pit[name] = _parse_pit(row, season)
                        added += 1
                print(f"[FG CSV] Pitching {season}: {added} players")
                csv_loaded = True
            except Exception as ex:
                print(f"[FG CSV] Pitching {season} failed: {ex}")

    # ── MLB Stats API fallback if no CSVs found ───────────────────────────────
    if not csv_loaded:
        print("[FG] No CSV files found — falling back to MLB Stats API")
        try:
            url = (f"https://statsapi.mlb.com/api/v1/stats?stats=season"
                   f"&group=hitting&season={year}&gameType=R&limit=800&playerPool=All")
            resp = requests.get(url, timeout=15); resp.raise_for_status()
            for sp in resp.json()["stats"][0]["splits"]:
                name = sp["player"]["fullName"].strip().lower()
                s = sp["stat"]
                pa  = _int(s.get("plateAppearances", 0))
                bb  = _int(s.get("baseOnBalls", 0))
                k   = _int(s.get("strikeOuts", 0))
                slg = _flt(s.get("slg", ".000"))
                avg = _flt(s.get("avg", ".000"))
                bat[name] = {
                    "fg_avg": avg, "fg_obp": _flt(s.get("obp",".000")),
                    "fg_slg": slg, "fg_ops": _flt(s.get("ops",".000")),
                    "fg_woba": 0.0, "fg_wrc": 0,
                    "fg_pa": pa, "fg_r": _int(s.get("runs",0)),
                    "fg_hr": _int(s.get("homeRuns",0)), "fg_rbi": _int(s.get("rbi",0)),
                    "fg_sb": _int(s.get("stolenBases",0)), "fg_war": 0.0,
                    "fg_babip": _flt(s.get("babip",".000")),
                    "fg_bbpct": round(bb/pa,3) if pa>0 else 0.0,
                    "fg_kpct":  round(k/pa,3)  if pa>0 else 0.0,
                    "fg_iso":   round(max(0.0,slg-avg),3), "fg_season": year,
                }
            print(f"[MLB API] Batting fallback: {len(bat)} players")
        except Exception as ex:
            print("[MLB API] Batting fallback failed:", ex)
        try:
            url = (f"https://statsapi.mlb.com/api/v1/stats?stats=season"
                   f"&group=pitching&season={year}&gameType=R&limit=800&playerPool=All")
            resp = requests.get(url, timeout=15); resp.raise_for_status()
            for sp in resp.json()["stats"][0]["splits"]:
                name = sp["player"]["fullName"].strip().lower()
                s = sp["stat"]
                bf = _int(s.get("battersFaced",0))
                bb = _int(s.get("baseOnBalls",0))
                k  = _int(s.get("strikeOuts",0))
                pit[name] = {
                    "fg_era": _flt(s.get("era","0")), "fg_fip": 0.0, "fg_xfip": 0.0,
                    "fg_whip": _flt(s.get("whip","0")),
                    "fg_k9":  _flt(s.get("strikeoutsPer9Inn","0")),
                    "fg_bb9": _flt(s.get("walksPer9Inn","0")),
                    "fg_hr9": _flt(s.get("homeRunsPer9","0")),
                    "fg_kpct":  round(k/bf,3)  if bf>0 else 0.0,
                    "fg_bbpct": round(bb/bf,3) if bf>0 else 0.0,
                    "fg_babip": _flt(s.get("avg",".000")), "fg_lob": 0.0, "fg_war": 0.0,
                    "fg_ip": _flt(s.get("inningsPitched","0")),
                    "fg_g": _int(s.get("gamesPlayed",0)), "fg_gs": _int(s.get("gamesStarted",0)),
                    "fg_w": _int(s.get("wins",0)), "fg_l": _int(s.get("losses",0)),
                    "fg_season": year,
                }
            print(f"[MLB API] Pitching fallback: {len(pit)} players")
        except Exception as ex:
            print("[MLB API] Pitching fallback failed:", ex)
...
def _derive_probs(b, p, park=1.0):
    ...
    # ML model blend: 25% ML, 75% formula (skipped if model not loaded)
    if hits_model is not None:
        try:
            _ml_feat = {
                "k_rate":  _clamp(0.175 + (k9 - 8.2) * 0.018, 0.09, 0.36),
                "bb_rate": _clamp(bb9 / 36.0, 0.04, 0.15),
                "babip":   avg,
                "slg":     slg,
                "obp":     obp,
                "hr_rate": _clamp(brl * 0.0028, 0.005, 0.095),
            }
            _df_ml = pd.DataFrame([_ml_feat]).reindex(columns=hits_features, fill_value=0)
            _ml_raw = float(hits_model.predict(_df_ml)[0])
            if math.isfinite(_ml_raw) and _ml_raw > 0:
                _ml_hitrate = _clamp(_ml_raw / 562.0, 0.13, 0.35)
                hit_rate = round(0.75 * hit_rate + 0.25 * _ml_hitrate, 4)
        except Exception:
            pass
