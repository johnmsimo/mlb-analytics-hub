    ...
    with ThreadPoolExecutor(max_workers=10) as pool:
        ...
        for fut in as_completed(futs):
            ...
            try:
                ...
                # Approximate FIP: (13*HR + 3*BB - 2*K)/IP + const(~3.2)
                so   = _safe_num(s.get("strikeOuts"), 0)
                bb   = _safe_num(s.get("baseOnBalls"), 0)
                hr   = _safe_num(s.get("homeRuns"), 0)
                fip  = round(((13*hr + 3*bb - 2*so) / ip + 3.20), 2) if ip > 0 else round(era, 2)
                pit_out[name.lower()] = {
                    "fg_era":   round(era, 2),
                    "fg_fip":   fip,
                    "fg_xfip":  fip,
                    ...
                    "fg_bb9":   round(bb9, 2),
                    "fg_hr9":   round(hr9, 2),
                    "fg_kpct":  round(so / max(_safe_num(s.get("battersFaced"), 1), 1), 3) if s.get("battersFaced") else 0.0,
                    "fg_bbpct": round(bb / max(_safe_num(s.get("battersFaced"), 1), 1), 3) if s.get("battersFaced") else 0.0,
                    "fg_babip": _safe_num(s.get("babip"), 0.290),
                    "fg_lob":   _safe_num(s.get("leftOnBasePercentage"), 0.72),
                    "fg_war":   0.0,
                    "fg_ip":    round(ip, 1),
    ...
    with ThreadPoolExecutor(max_workers=10) as pool:
        ...
        for fut in as_completed(futs):
            ...
            try:
                ...
                so   = _safe_num(s.get("strikeOuts"), 0)
                bb   = _safe_num(s.get("baseOnBalls"), 0)
                # Approximate wOBA: ((OBP - 0.320) * 0.8 + 0.320) is a
                # reasonable proxy when FG isn't available.
                woba = round(obp * 0.9 + slg * 0.1, 3) if obp > 0 else 0.320
                bat_out[name.lower()] = {
                    "fg_avg":   round(avg, 3),
                    "fg_obp":   round(obp, 3),
                    "fg_slg":   round(slg, 3),
