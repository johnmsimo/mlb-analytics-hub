...
def _maybe_refresh_savant():
    ...
    if not loaded or date != datetime.now().date():
        ...
        def _runner():
            global _sv_loading
            try: _load_savant_data()
            finally:
                with _sv_lock: _sv_loading = False
        threading.Thread(target=_runner, daemon=True).start()
...
def api_simulate(game_pk):
    try:
        ...
        # Boxscore first, roster fallback for pre-game
        away_lineup, home_lineup = [], []
        try:
            box = requests.get(f"{MLB_API}/game/{game_pk}/boxscore", timeout=8).json().get('teams', {})
            away_lineup = [b for b in get_batters_from_boxscore(box.get('away', {}), 'away') if b.get('name')]
            home_lineup = [b for b in get_batters_from_boxscore(box.get('home', {}), 'home') if b.get('name')]
        except Exception as ex:
            print(f"[simulate] boxscore error: {ex}")

        def _roster_batters(team_id):
            try:
                r = requests.get(f"{MLB_API}/teams/{team_id}/roster?rosterType=active", timeout=6)
                r.raise_for_status()
                out = []
                for e in (r.json().get('roster') or []):
                    ...
                    if not name: continue
                    fgb = fg_batter(name)
                    out.append({'name': name, 'slot': 0, 'pos': pos,
                        'avg': fgb.get('fg_avg', '.---'), 'obp': fgb.get('fg_obp', '.---'),
                        'slg': fgb.get('fg_slg', '.---'), 'bats': fgb.get('fg_bats', 'R'),
                        'fg_woba': fgb.get('fg_woba'), 'fg_wrc': fgb.get('fg_wrc'),
                    })
                return out[:15]
            except Exception as ex:
                print(f"[simulate] roster error {team_id}: {ex}")
                return []

        if not away_lineup: away_lineup = _roster_batters(away_team_id)
        if not home_lineup: home_lineup = _roster_batters(home_team_id)

        if not away_lineup or not home_lineup:
            return jsonify({'success': False, 'error': 'Could not load lineups for simulation'}), 400
...
def _mc_lineup_from_schedule(game_data, side):
    """Extract lineup from schedule hydrate=lineups response. Returns list of batter dicts."""
    lineups = game_data.get('lineups') or {}
    key = 'awayBatters' if side == 'away' else 'homeBatters'
    hitters = lineups.get(key) or []
    if not hitters:
        return []
    out = []
    for slot_i, p in enumerate(hitters, start=1):
        name = (p.get('fullName') or p.get('name') or '').strip()
        if not name: continue
        fgb = fg_batter(name)
        svb = sv_batter(name)
        out.append({
            'name': name, 'slot': slot_i, 'side': side,
            'pos': ((p.get('primaryPosition') or {}).get('abbreviation') or '?'),
            'fg_avg': fgb.get('fg_avg'), 'fg_obp': fgb.get('fg_obp'),
            'fg_slg': fgb.get('fg_slg'), 'fg_woba': fgb.get('fg_woba'),
            'fg_wrc': fgb.get('fg_wrc'), 'fg_kpct': fgb.get('fg_kpct'),
            'sv_xba': svb.get('sv_xba'), 'sv_xwoba': svb.get('sv_xwoba'),
            'sv_brl_pct': svb.get('sv_brl_pct'), 'sv_ev': svb.get('sv_ev'),
        })
    return out

def _mc_lineup_from_boxscore(game_pk, side):
    """Boxscore fetch for a single side — only called as fallback."""
    try:
        r = requests.get(f"{MLB_API}/game/{game_pk}/boxscore", timeout=6)
        r.raise_for_status()
        box = r.json().get('teams', {})
        batters = get_batters_from_boxscore(box.get(side, {}), side)
        named = [b for b in batters if b.get('name')]
        for b in named: b['side'] = side
        return named
    except Exception as ex:
        print(f"[mc] pk={game_pk} {side} boxscore err: {ex}")
        return []

def _mc_lineup_from_roster(team_id, side):
    """Roster fallback — always has data, used when no lineup posted."""
    try:
        r = requests.get(f"{MLB_API}/teams/{team_id}/roster?rosterType=active", timeout=5)
        r.raise_for_status()
        out = []
        for e in (r.json().get('roster') or []):
            pos = ((e.get('position') or {}).get('abbreviation') or '?')
            ...
            out.append({'name': name, 'slot': 0, 'side': side, 'pos': pos,
                'fg_avg': fgb.get('fg_avg'), 'fg_obp': fgb.get('fg_obp'),
                'fg_slg': fgb.get('fg_slg'), 'fg_woba': fgb.get('fg_woba'),
                'fg_wrc': fgb.get('fg_wrc'), 'fg_kpct': fgb.get('fg_kpct'),
                'sv_xba': svb.get('sv_xba'), 'sv_xwoba': svb.get('sv_xwoba'),
        ...
        print(f"[mc] {side} roster {team_id} err: {ex}")
        return []

def _mc_props_for_batter(b, matchup):
    name = b.get('name', ''); side = b.get('side', '')
    if not name: return []
    svb = sv_batter(name); fgb = fg_batter(name)
    slot = b.get('slot') or 5
    try: xba    = float(svb.get('sv_xba')    or fgb.get('fg_avg')  or b.get('fg_avg')  or 0.250)
    except: xba = 0.250
    try: woba   = float(svb.get('sv_xwoba')  or fgb.get('fg_woba') or b.get('fg_woba') or 0.320)
    except: woba = 0.320
    try: brl    = float(svb.get('sv_brl_pct') or b.get('sv_brl_pct') or 0)
    except: brl  = 0.0
    try: ev     = float(svb.get('sv_ev')      or b.get('sv_ev')      or 88.0)
    except: ev   = 88.0
    try: fg_slg = float(fgb.get('fg_slg') or b.get('fg_slg') or 0.380)
    except: fg_slg = 0.380
    try: wrc    = int(fgb.get('fg_wrc')   or b.get('fg_wrc')   or 100)
    except: wrc  = 100
    try: obp    = float(fgb.get('fg_obp') or b.get('fg_obp')   or 0.320)
    except: obp  = 0.320
    lm = 1.06 if slot <= 2 else (1.03 if slot <= 4 else (0.97 if slot >= 8 else 1.0))
    pa_hit = min(0.92, xba  * 3.2 * lm)
    pa_tb  = min(0.88, woba * 2.4 * lm)
    pa_hr  = min(0.50, brl  / 100 * 1.8 * lm)
    pa_rbi = min(0.72, woba * 2.0 * lm)
    props = []
    for mk, line, prob, reason in [
        ('batter_hits',        0.5, pa_hit,
         f"xBA {xba:.3f} · wOBA {woba:.3f} · wRC+ {wrc} → {pa_hit*100:.0f}% hit prob; bats {slot} ({side})"),
        ('batter_total_bases', 1.5, pa_tb,
         f"xwOBA {woba:.3f} · SLG {fg_slg:.3f} · EV {ev:.1f} mph → {pa_tb*100:.0f}% TB prob vs 1.5"),
        ('batter_home_runs',   0.5, pa_hr,
         f"Barrel% {brl:.1f} · EV {ev:.1f} mph · wOBA {woba:.3f} → {pa_hr*100:.0f}% HR prob vs 0.5"),
        ('batter_rbis',        0.5, pa_rbi,
         f"wOBA {woba:.3f} · OBP {obp:.3f} · bats {slot} ({side}) → {pa_rbi*100:.0f}% RBI prob vs 0.5"),
    ]:
        if prob < 0.20: continue
        edge = round(prob - 0.50, 4)
        props.append({'player': name, 'market': mk, 'line': line,
            'bookmaker': 'Model', 'price': None, 'source': 'simulation', 'matchup': matchup,
            'edge': edge, 'prob': round(prob, 4),
            'grade': _mc_grade(edge), 'recommendation': _mc_rec(edge), 'reasoning': reason})
    return props

def _mc_props_for_pitcher(pit_info, matchup):
    pname = (pit_info.get('fullName') or '').strip()
    if not pname: return []
    ...
    if k_prob < 0.30: return []
    edge = round(k_prob - 0.50, 4)
    return [{'player': pname, 'market': 'pitcher_strikeouts', 'line': 5.5,
        'bookmaker': 'Model', 'price': None, 'source': 'simulation', 'matchup': matchup,
        'edge': edge, 'prob': round(k_prob, 4),
        'grade': _mc_grade(edge), 'recommendation': _mc_rec(edge), 'reasoning': reason}]
...
def api_projections_monte_carlo():
    ...
    Strategy: ONE schedule call with hydrate=lineups gets all lineups simultaneously.
    Only falls back to per-game boxscore or roster if schedule lineups are missing.
    """
    _maybe_refresh_fg()
    _maybe_refresh_savant()
    try:
        ...
        # ── SINGLE API CALL: schedule + lineups for all games ─────────────
        url = (f"{MLB_API}/schedule?sportId=1&date={date_str}"
        ...
        try:
            ...
            print(f"[mc] schedule fetch error: {ex}")
            return jsonify({'success': False, 'error': f'Schedule fetch failed: {ex}',
                            'games': [], 'topProps': []}), 500

        if not raw:
            return jsonify({'success': True, 'date': date_str, 'hasOdds': False,
                            'games': [], 'topProps': []})

        has_odds = bool(ODDS_API_KEY)
        games, ranked = [], []

        # Identify games that need fallback fetches
        needs_fallback = []  # (game_pk, away_tid, home_tid, sides_needed)
        game_lineups = {}    # game_pk → {away: [...], home: [...]}
        ...
        for g in raw:
            ...
            away_lu = _mc_lineup_from_schedule(g, 'away')
            home_lu = _mc_lineup_from_schedule(g, 'home')

            if len(away_lu) >= 5 and len(home_lu) >= 5:
                game_lineups[game_pk] = {'away': away_lu, 'home': home_lu}
                print(f"[mc] pk={game_pk}: schedule lineups {len(away_lu)}a/{len(home_lu)}h")
            else:
                needs_missing = []
                if len(away_lu) < 5: needs_missing.append(('away', away_tid))
                if len(home_lu) < 5: needs_missing.append(('home', home_tid))
                game_lineups[game_pk] = {'away': away_lu, 'home': home_lu}
                needs_fallback.append((game_pk, needs_missing))

        # ── Parallel roster fetches ONLY for missing lineups ─────────────
        if needs_fallback:
            # First try boxscore for each game with missing sides (one call per game)
            def _fallback_game(game_pk, missing_sides):
                result = {}
                for side, tid in missing_sides:
                    # Try boxscore first (covers live/recent games)
                    bx = _mc_lineup_from_boxscore(game_pk, side)
                    if len(bx) >= 5:
                        result[side] = bx
                    else:
                        # Last resort: roster
                        result[side] = _mc_lineup_from_roster(tid, side)
                return game_pk, result

            with ThreadPoolExecutor(max_workers=8) as pool:
                futs = {pool.submit(_fallback_game, pk, sides): pk
                        for pk, sides in needs_fallback}
                for fut in as_completed(futs):
                    try:
                        pk, result = fut.result()
                        for side, lu in result.items():
                            game_lineups[pk][side] = lu
                        print(f"[mc] pk={pk} fallback: "
                              + " ".join(f"{s}={len(l)}" for s,l in result.items()))
                    except Exception as ex:
                        print(f"[mc] fallback error: {ex}")

        # ── Build props (pure Python, no HTTP) ───────────────────────────
        for g in raw:
            game_pk   = g.get('gamePk')
            away_t    = g.get('teams', {}).get('away', {})
            home_t    = g.get('teams', {}).get('home', {})
            ...
            try:
                lu = game_lineups.get(game_pk, {})
                away_lineup = lu.get('away', [])
                home_lineup = lu.get('home', [])
                all_batters = away_lineup + home_lineup
                print(f"[mc] {matchup}: {len(away_lineup)}a + {len(home_lineup)}h batters")

                if has_odds:
                    valid_names = {b.get('name') for b in all_batters if b.get('name')}
                    ...
                    for p in props:
                        ...
                        scored.append({'player': p.get('player'), 'market': p.get('market_key'),
                    ...
                    for b in all_batters:
                        top_props.extend(_mc_props_for_batter(b, matchup))
                    for pit in [away_p, home_p]:
                        top_props.extend(_mc_props_for_pitcher(pit, matchup))
                    top_props = sorted(top_props, key=lambda x: x['edge'], reverse=True)

                ranked.extend(top_props)
            except Exception as ex:
                print(f"[mc] {matchup}: {traceback.format_exc()}")
        ...
        return jsonify({'success': True, 'date': date_str, 'hasOdds': has_odds,
                        'games': games, 'topProps': ranked[:500]})
    except Exception as ex:
        print(f"[api_mc] {traceback.format_exc()}")
