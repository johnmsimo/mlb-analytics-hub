...
- **MLB memory-store views + one-pass appends**: the ~12MB-and-growing `mlb_memory_store.json` is parsed once per device/inode/mtime/size file version. `/api/memory/status` and `summaryOnly` latest reads use precomputed small views instead of cloning the whole seasonal store; full-store and full-latest callers receive private pickle copies. Three-hour collection writes reuse the warm parsed payload, preserve the existing retention/compaction schema, encode the final byte-limited store once, atomically replace it, and immediately advance every cached view without reparsing. Failed replacements leave the prior file/cache intact. A representative append on the checked-in 11.9MB store improved from ~339ms to ~114ms (3×) with equivalent JSON data.
