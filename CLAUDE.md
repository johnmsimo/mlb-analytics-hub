...
- **MLB latest-response representations** (`_mlb_memory_latest_response`): full and summary `/api/memory/latest` responses lazily encode JSON and gzip once per memory-store file version, then reuse immutable response bytes across requests. Strong ETags allow bodyless 304 revalidation, inode-aware file changes and successful appends discard stale representations automatically, and the decoded API schema is unchanged. On the checked-in 594KB latest snapshot, 100 repeated gzip-enabled route requests improved from ~1.54s to ~0.036s (15.36ms → 0.36ms each, ~42.7×) with byte-identical decoded JSON.
- **Tracker day-response representations** (`_tracker_api_response`): `/api/tracker/today`, `/api/tracker/date/<date>`, and `/api/tracker/entries` lazily build bounded immutable JSON/gzip responses keyed by both tracker-store and adjustment-file versions. Concurrent cold requests encode once, strong ETags allow bodyless 304 revalidation, atomic pick writes and settings replacements invalidate automatically, and decoded schemas remain unchanged. A representative 250-row serialization benchmark reduced JSON/gzip work from ~1.26ms per request to an immutable byte lookup.
- **HTML pages: ETag + gzip** (`_page_response`): every page route serves through this helper. `Cache-Control: no-cache` + strong ETag replaces the old `no-store` — the browser still revalidates every navigation (frontend edits show up without redeploy), but an unchanged page gets a bodyless 304 and a changed one ships gzipped (deepdive 423KB→107KB) from a content-hash-keyed compression cache.
