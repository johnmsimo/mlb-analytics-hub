...
├── stuff_model.py              Physics-based pitch-quality ("Stuff+") model — pitch-type XGB regressor predicting expected whiff from velo/movement/spin/extension ONLY (no outcomes), scaled to league 100 ±10/SD. Shared by regenerate_models.py (training aggregates + the models/stuff_whiff.pkl artifact) and app.py (`_stuff_plus_for`: live current-season score per starter, day-cached, prior-season fallback below the 150-pitch floor). Pure-Python, self-tested (`python stuff_model.py`)
...
├── clients/mlb_client.py        Pooled, retrying MLB Stats API boundary
├── mlb_schedule_cache.py        Shared date/game schedule cache
...
- `/api/projections/monte-carlo` — dashboard Monte Carlo board (slate-wide top props + per-game groups). Backed by the cached props-scan (same source as `/api/edges/today`), so the board shows the real calibrated pipeline — `adjProb` and capped edge vs the best available book price, recommending whichever side (over/under) carries the value — instead of the old standalone background compute, whose odds-path "edge" was just the de-vig margin (no model input at all). Never blocks: `?refresh=1` kicks an async scan refresh and returns the current snapshot immediately, and the startup projections prewarm chains a props-scan warm so the first dashboard visit of the day serves instantly. Model-only rows (no book line) are graded against per-market base rates (`_MC_NOODDS_BASE_RATE`), not the old flat 0.50
...
**Physics-based Stuff+ (`stuff_model.py`).** The K models train on `stuff_plus`: a pitch-TYPE-level XGB regressor predicts each pitch type's whiff rate from PHYSICS ONLY (velo, arm-side-normalized pfx_x, pfx_z, spin, extension, velo gap off the primary fastball), and a pitcher's score is his arsenal's usage-weighted expected whiff scaled to Stuff+ (league 100, ±10/SD). Unlike the outcome-based SwStr%/arsenal-whiff features it stabilizes in ~150 pitches and leads outcomes when a pitcher adds velo or a pitch. Leakage-safe: the physics→whiff mapping is fit on TRAIN seasons only; pitcher-season scores follow the same same-season-aggregate convention as the FG stats. Parity by construction: training aggregates and the live path run the SAME `stuff_model` functions (`physics_sums → finalize_features → score_groups`) on the same raw Statcast source — training via `regenerate_models._agg_chunk`, serve via `app._stuff_plus_for(pitcher_id)` (current-season `statcast_pitcher` pull, day-cached, prior-full-season fallback below the 150-pitch floor). Missing score = NaN, never imputed (same policy as `bt_*`). Call sites gate on `xgb_feature_in_use('stuff_plus')` so the pull costs nothing until a retrained K model actually consumes it; the artifact is `models/stuff_whiff.pkl`, produced by the weekly regen. Sanity benchmark (6-week 2024 fit): Skenes 118 / Skubal 112 / Crochet 112 vs Kirby 97 — physics ordering recovered with no outcome features. **The batter markets consume it too:** HR/TB/RBI train on `opp_stuff_plus` (the OPPOSING starter's Stuff+, joined by the same real (season, opp_starter) key as the opp_* FG stats — contact/damage suppression the outcome-based opp_hr9/opp_barrel lag on). Serve reads `stuffPlus` off the opposing-pitcher dict in `_build_batter_market_features`; `_maybe_opp_stuff` attaches it (gated on `xgb_feature_in_use('opp_stuff_plus', 'hr'/'tb'/'rbi')`) at the deep-dive/AI-boxscore, tracker-capture, lineup-props and quick-props paths — once per starter per day, so the batter markets add zero extra Statcast pulls beyond the K path's.
...
- **Pooled HTTP session** (`_HTTP_SESSION`, app.py top): module-level `requests.get` is rebound to a keep-alive `Session.get`. A fresh TCP+TLS handshake to statsapi.mlb.com measured 500-700ms per call and the app makes thousands per hour; connection reuse is the single biggest systemic win. Endpoint-aware resilient caching also deduplicates schedule, team-venue, boxscore/live-feed, player/stat, roster, standings, and transaction GETs with query-isolated keys and `LIVE`/`SCHEDULE`/`STATS`/`STATIC` TTL policies. Raw schedule reads retain the dedicated 120-second freshness contract. DraftKings MLB and The Odds API MLB reads use the same query isolation, singleflight, and stale fallback with their existing five-minute TTLs; unrelated external hosts remain uncached. Anything importing `requests` in-process (pybaseball, brain) shares the pool.
...
- **Indexed primary FanGraphs lookups** (`fangraphs_loader.py`): every batting, pitching, and projection season dataframe builds an immutable ID/name index once at load. The result memo still removes repeated fallback/merge work, while first-time unique-player lookups avoid pandas boolean scans across the six-season chain and preserve partial-name, minimum-sample, and prior-season merge behavior.
...
- **Shared game-data cache** (`http_client.py`) — all boxscore and v1.1 live-feed consumers share the pooled session plus the `CACHE_TTL_LIVE` policy. Concurrent misses compute once, Redis/in-memory backends share the response, and stale-if-error protects projections, tracker grading, lineups, bullpen, and AI views during short upstream outages.
...
- **Precomputed Savant bat-speed percentiles** (`savant_bat_tracking.py`): the 24-hour dataframe refresh now ranks each distinct bat speed once, matching the previous `speed <= player speed` percentile exactly. BATX and XGBoost feature enrichment reuse the immutable value table instead of scanning the full leaderboard for every batter; dataframe refreshes rebuild the row index and percentile table together under one lock.
- **Indexed umpire snapshots** (`umpire_loader.py`): historical Savant/fallback features and daily home-plate assignments are parsed once per mtime-stamped file version. Exact, last-name, and first-name-prefix indexes preserve the legacy first-match order, concurrent cold reads collapse under a lock, assignment results are copy-isolated, and failed Savant refreshes use a bounded retry cooldown.
- **Weather snapshots** (`weather_loader.py`): each date's three-hour weather file is parsed once per mtime-stamped version and reused for per-player feature lookups. Concurrent cold reads share one schedule/Open-Meteo refresh, date snapshots are bounded, external file replacements invalidate automatically, and callers receive mutation-isolated dictionaries.
- **Per-day tracker snapshots + copy-on-write** (`_tracker_cached_snapshot` / `_tracker_commit_day`): `daily_tracker.json` is parsed once per device/inode/mtime/size file version into immutable full-store and per-day pickle/JSON blobs. Read-only day/window routes deserialize only the requested dates instead of cloning the multi-MB season store. Single-day mutations reuse the cached JSON for every unchanged date, atomically replace only the changed day's encoded value, and immediately advance the in-memory snapshot; full-history readers still receive private mutable stores rebuilt lazily when needed. Concurrent cold reads collapse under one lock, concurrent writes remain serialized, atomic replacements invalidate deterministically, and failed reads/writes do not poison the last valid file. A representative 12.3MB/180-day store made repeated 14-day reads ~17× faster and 12 sequential single-day writes ~5.1× faster with identical data. `_save_json` also writes compact separators instead of `indent=2` (~37% fewer bytes through fsync on every save).
- **Request-local tracker analytics snapshots**: performance, calibration, and value dashboards clone the season tracker once per response, then pass that private snapshot through every market/day series builder. Calibration history is loaded once per response, bulk pick recalculation shares one adjustment snapshot, and `/api/tracker/entries` uses the same mtime-aware tracker boundary instead of reparsing the JSON file. A representative 22,500-entry, 14-market dashboard build improved from ~190ms to ~42ms (4.5×).
- **Tracker analytics-response representations** (`_tracker_analytics_response`): `/api/tracker/performance`, `/api/tracker/calibration/dashboard/<date>`, and `/api/tracker/value/dashboard/<date>` encode JSON and gzip once per date/window and tracker/control-file version, then reuse immutable bytes across refreshes. Encoding-specific strong ETags make unchanged matching refreshes bodyless; tracker writes plus adjustment/history replacements invalidate automatically without changing decoded schemas.
- **Tracker day-response representations** (`_tracker_day_response`): `/api/tracker/date/<date>`, `/api/tracker/today`, and `/api/tracker/entries` encode JSON and gzip once per tracker/adjustment file version, then reuse immutable bytes across refreshes. Encoding-specific strong ETags return bodyless 304 responses only for the matching identity or gzip bytes, game filters remain isolated, and successful day commits plus inode-aware external replacements invalidate without changing decoded API contracts.
- **Control-file snapshots**: `model_adjustments.json` and `calibration_history.json` are parsed once per device/inode/mtime/size file version and reused across requests. Concurrent cold reads collapse under one lock, atomic/external file replacements invalidate automatically, and callers receive private pickle-round-trip copies so nested mutations cannot leak between requests.
- **MLB memory-store views + per-snapshot appends**: the ~12MB-and-growing `mlb_memory_store.json` is parsed once per device/inode/mtime/size file version into immutable per-snapshot pickle blobs plus lightweight status/latest views. `/api/memory/status` and `summaryOnly` latest reads never materialize the full seasonal store; full-store callers lazily build one shared immutable pickle under singleflight and still receive private mutable copies. Three-hour collection writes reuse canonical JSON for unchanged retained snapshots, so a warm append transforms only the new snapshot and the one aging from detailed to compact history before atomically replacing the byte-limited store. Cache views advance immediately, failed replacements leave the prior file/cache intact, and retention/compaction output is unchanged. On identical copies of the checked-in 11.9MB store, repeated warm appends improved from ~173ms to ~90ms (1.9×) with byte-equivalent retained data.
- **MLB latest-response representations** (`_mlb_memory_latest_response`): full and summary `/api/memory/latest` responses lazily encode JSON and gzip once per memory-store file version, then reuse immutable response bytes across requests. Encoding-specific strong ETags allow bodyless 304 revalidation only for the matching identity or gzip bytes, inode-aware file changes and successful appends discard stale representations automatically, and the decoded API schema is unchanged. On the checked-in 594KB latest snapshot, 100 repeated gzip-enabled route requests improved from ~1.54s to ~0.036s (15.36ms → 0.36ms each, ~42.7×) with byte-identical decoded JSON.
- **HTML pages: ETag + gzip** (`_page_response`): every page route serves through this helper. `Cache-Control: no-cache` + encoding-specific strong ETags replace the old `no-store` — the browser still revalidates every navigation (frontend edits show up without redeploy), but an unchanged matching variant gets a bodyless 304 and a changed one ships gzipped (deepdive 423KB→107KB) from a content-hash-keyed compression cache.
