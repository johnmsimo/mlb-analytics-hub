...
**XGB / stacked-calibrator fields (hits/hr/tb/rbi at their model line).** `_build_tracker_rows_for_game` runs the XGB point probability and the analytic/MC probability through `stacked_calibrator.calibrate(market_key=mk)` and persists `xgbProb`, `batxProb`, `stackedProb`, `stackCiLo`/`stackCiHi` (plus `stackVerdict`/`stackVerdictLabel` for **hits only** — the verdict TIER thresholds are calibrated to the ~0.66 hits base rate and would mislabel TB/HR/RBI, whose base rates are ~0.33/0.11/0.29; the prob + CI are valid for all four). This does **not** change `adjProb`/`edge` (still the tuned MC + market-blend + `prop_calibration` pipeline) — the stacked fields are recorded alongside. The persisted `xgbProb`/`batxProb` are exactly what `stacked_calibrator.update_model_accuracies` reads to drive Smart-Consensus; `_tracker_auto_sync_once` calls it after each grade pass, so once ≥40 graded picks per market carry both probs the blend shifts from the coverage heuristic to measured per-model Brier.
