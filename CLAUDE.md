...
- **Per-day tracker snapshots** (`_tracker_cached_snapshot`): `daily_tracker.json` is parsed once per device/inode/mtime/size file version into immutable full-store and per-day pickle blobs. Read-only day/window routes deserialize only the requested dates instead of cloning the multi-MB season store, while writers and all-date exports retain private mutable full reads. Concurrent cold reads collapse under one lock, atomic replacements invalidate deterministically, and failed reads do not poison a repaired file. A representative 12.3MB/180-day store made repeated 14-day reads ~17× faster with identical data. `_save_json` also writes compact separators instead of `indent=2` (~37% fewer bytes through fsync on every save).
