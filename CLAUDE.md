...
- **MLB memory-store views**: the ~12MB-and-growing `mlb_memory_store.json` is parsed once per device/inode/mtime/size file version. `/api/memory/status` and `summaryOnly` latest reads use precomputed small views instead of cloning the whole seasonal store; full-store writers and full latest reads receive private pickle copies. Concurrent cold reads collapse under one lock and atomic replacements invalidate automatically.
