...
**"Heater" momentum (true-talent latency).** `regenerate_models.py` computes per-game contact quality from raw Statcast (`g_ev` = mean exit velo on batted balls, `g_hh` = ≥95mph share, `g_barrel` = `launch_speed_angle==6` share, `g_whiff` = swinging-strike/swing) and rolls them into recent-form levels (`l7_ev`/`l7_barrel`/…) plus recent-vs-30g-baseline ratios (`ev_momentum`/`barrel_momentum`, >1 = heating up). These are served live by `app._batter_momentum_features(batter_id)` — same Statcast source (`statcast_batter`), same aggregation, same scale → no train/serve skew. The serve defaults in `_build_hr_features` are the recorded HR `train_medians` so an un-pulled batter is the neutral case. **Momentum is in the HR model only:** tested on the ≥1-hit market it was flat (held-out AUC 0.6221 vs 0.6223 — recent contact quality doesn't beat the season EV/HH the model already has for "gets a hit"), so it was not shipped there. In the HR model it is a real but minor positive contributor (importance ranks 20–24 of 24; HR is dominated by season ISO/HR-FB + lineup volume + opp HR/9). Momentum is pulled only on the deep-dive / AI-boxscore path (where `xgb_hr_prob` is called with a batter id), not the name-only slate scan, so it costs one daily-cached Statcast lookup per batter.
...
**HR is committed; TB/RBI are still deferred.** `xgb_hr_over_0.5.pkl` is now a real, self-calibrated `regenerate_models.py` artifact (`--markets hr`, held-out AUC 0.68), scored through its own `_build_hr_features` builder (HR uses different features than hits, so `_predict_batter_market_full` selects the builder by `model_key`). `xgb_tb_over_1.5.pkl` and `xgb_rbi_over_0.5.pkl` are still absent — the scorer skips missing files (`if not os.path.exists(path): continue`), so `xgb_tb_prob / xgb_rbi_prob` return `None` and TB/RBI fall back to the analytic `_project_batter` model (expected, not a bug). To enable them, add markets to `regenerate_models.py` (mirroring the `hr` entry) and validate held-out before committing under the pinned `xgboost==3.2.0` / `scikit-learn==1.6.1`.
