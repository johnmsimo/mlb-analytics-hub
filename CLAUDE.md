...
- **Tracker store read cache** (`_tracker_store`): `daily_tracker.json` grows all season and one tracker page load fires ~6 API calls that each re-parsed it; the parsed store is now cached against an (mtime_ns, size) signature and each call returns a private pickle-round-trip copy (~3.5× faster than re-parsing, zero disk). Any write (in-process `_save_json` atomic replace or an external tool) changes the signature and forces a re-parse — callers may still freely mutate what they get back. `_save_json` also writes compact separators instead of `indent=2` (~37% fewer bytes through fsync on every save; the ~12MB `mlb_memory_store.json` is rewritten whole every collection cycle).
